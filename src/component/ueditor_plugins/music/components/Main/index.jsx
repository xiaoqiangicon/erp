...
export default class Main extends Component {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render(text, record, index) {
          return (
            <button
              className={`clean ${styles.play}`}
              onClick={self.onClickPlay.bind(self)}
              data-item-play={index}
            />
          );
        },
      },
  ...
  onClickPlay(e) {
    const $this = $(e.target);
    const index = parseInt($this.attr('data-item-play'));
  ...
  getContentJsx() {
    const { noSearch, error, requesting, noData } = this.state;
