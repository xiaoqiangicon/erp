...
let requestList = (instanceId, page, init) => {
  ...
  let $listContainer = $chooseIcon.find("[data-zzh-choose-icon-pagination-container]");
  let $paginationContainer = $chooseIcon.find("[data-zzh-choose-icon-pagination]");
  $listContainer.html(loadingTpl);
  init && $paginationContainer.html("");
  seeAjax("zzhChooseIconIcons", {
    page
  }, res => {
