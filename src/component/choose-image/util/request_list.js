...
let requestList = (instanceId, page, init) => {
  ...
  let $listContainer = $chooseImage.find("[data-zzh-choose-image-pagination-container]");
  let $paginationContainer = $chooseImage.find("[data-zzh-choose-image-pagination]");
  ...
  seeAjax(activeTab === 1 ? "zzhChooseImageSavedImages" : "zzhChooseImageSystemImages", {
    page
  }, res => {
    if (!res.success || !res.data || !res.data.length) {
      if (page === 1 && activeTab === 1) {
        $chooseImage.find("[data-zzh-choose-image-tab-1=\"2\"]").trigger("click");
        $chooseImage.find("[data-zzh-choose-image-to-manage-container]").hide();
      } else {
