...
seeView({
  ...
  onClickTab1: e => {
    ...
    let $deleteContainer = $chooseImage.find("[data-zzh-choose-image-to-manage-container]");
    if (activeTab === 2) $deleteContainer.show(); else $deleteContainer.hide();
    requestList(id);
  },
  onClickCell: e => {
    ...
    if (inDeleteMode) {
      ...
      if (option.multiSelect) $this.toggleClass("active"); else {
  ...
  onClickAdd: function (e) {
    ...
    if ($addPopup.length) $addPopup.show(); else {
      $addPopup = $(addPopupTpl({
        id: id,
  ...
  afterAddPopup: ($addPopup, id, option) => {
    upload($addPopup.find("[data-zzh-choose-image-upload-button-inner]")[0], (url, e, data) => {
      $addPopup.find("[data-zzh-choose-image-progress-container]").remove();
      let $uploadEl = $(e.target).parents("[data-zzh-choose-image-upload-button]");
      let $imageCell = $(uploadImageCellTpl({
        image: url
      }));
      $uploadEl.before($imageCell);
    }, (e, data) => {
      let progress = parseInt(data.loaded / data.total * 100, 10), $progressBar = $addPopup.find("[data-zzh-choose-image-progress-bar]"), $progressBarContainer = $addPopup.find("[data-zzh-choose-image-progress-container]");
      if (!$progressBar.length) {
        let $uploadEl = $(e.target).parents("[data-zzh-choose-image-upload-button]");
        $progressBarContainer = $(progressTpl);
        $uploadEl.before($progressBarContainer);
        $progressBar = $progressBarContainer.find("[data-zzh-choose-image-progress-bar]");
      }
      $progressBar.css({
        width: progress + "%"
      });
