...
seeView({
  ...
  onClickCancelManage: e => {
    ...
    $chooseImage.find("[data-zzh-choose-image-selected-count]").text(0);
    $chooseImage.find("[data-zzh-choose-image-cell].delete-active").removeClass("delete-active");
  ...
  onClickDeleteSelected: e => {
    ...
    let activeTab = parseInt($chooseImage.find("[data-zzh-choose-image-tab-1].active").attr("data-zzh-choose-image-tab-1"));
    let $imagesCells = $chooseImage.find("[data-zzh-choose-image-cell].delete-active");
    let ids = [];
    if (activeTab === 2) {
    ...
    fn("确定要删除吗", _ => {
      seeAjax("zzhChooseImageDelete", {
        ids: JSON.stringify(ids)
      }, res => {
        if (res.success) {
          if ($imagesCells.length === $chooseImage.find("[data-zzh-choose-image-cell]").length) requestList(id); else $imagesCells.remove();
  ...
  onChangeSelectAll: e => {
    ...
    if (selected) $cells.addClass("delete-active"); else $cells.removeClass("delete-active");
