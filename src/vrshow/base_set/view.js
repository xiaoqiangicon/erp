...
$.seeView({
  ...
  changeQrcode: function (e) {
    var $tar = $(e.target), $qrcodeContainer = $("#qrcode-container"), type = parseInt($tar.attr("data-switch-qrcode"));
  ...
  playMusic: function (e) {
    var $tar = $(e.target), $audio = $("audio"), $curPlayMusic = $("[data-music-play=" + 1 + "]"), audio = $audio.get(0), src = $tar.attr("data-src"), curSrc = $audio.prop("src");
  ...
  saveTempleSet: function (e) {
    data.updateTempleSetParams.giftList.map(function (item) {
      var content = $("[data-input-day-type=" + item.type + "]").val(), formType = parseInt($("[data-select-day-type=" + item.type + "]").val());
      item.content = content;
      item.formType = formType;
    });
    var $limitRadio = $("#free-incense-limit"), $limitNumInput = $("#limit-num"), limit = $limitRadio.prop("checked"), num = parseInt($limitNumInput.val());
    if (limit && isNaN(num)) {
    ...
    $("#music-list-container").find("[type=\"radio\"]").each(function (index, ele) {
      if ($(ele).prop("checked")) {
