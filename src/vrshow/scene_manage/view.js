...
$.seeView({
  ...
  onClickSummaryTab: function (e) {
    e.preventDefault();
    var $tar = $(e.target), index = parseInt($tar.attr("data-index"));
  ...
  onClickLinkAddBtn: function (e) {
    var $modal = $("#add-link-modal"), $sceneSelect = $modal.find("[data-ele=\"scene-select\"]"), $linkTextarea = $modal.find("[data-ele=\"link-textarea\"]");
    if ($sceneSelect.html() === "") {
  ...
  onClickAddLinkSubmit: function (e) {
    var $modal = $("#add-link-modal"), $sceneSelect = $modal.find("[data-ele=\"scene-select\"]"), $linkTextarea = $modal.find("[data-ele=\"link-textarea\"]"), $title = $modal.find("[data-ele=\"link-title\"]"), sceneId = parseInt($sceneSelect.val()), title = $title.val(), url = $linkTextarea.val();
  ...
  onClickEditLinkSubmit: function (e) {
    var $modal = $("#edit-link-modal"), id = parseInt($modal.attr("data-id")), $sceneSelect = $modal.find("[data-ele=\"scene-select\"]"), $linkTextarea = $modal.find("[data-ele=\"link-textarea\"]"), $title = $modal.find("[data-ele=\"link-title\"]"), sceneId = parseInt($sceneSelect.val()), title = $title.val(), url = $linkTextarea.val();
    if (url.indexOf("zizaihome.com") === -1) {
      Toast("请输入自在家平台的佛事广告位", 0, 2000, 0);
      return false;
    }
  ...
  onClickEditLinkPreview: function (e) {
    var self = this, $modal = $("#edit-link-modal"), $linkTextarea = $modal.find("[data-ele=\"link-textarea\"]"), url = $linkTextarea.val();
    if (url.indexOf("zizaihome.com") === -1) {
      Toast("请输入自在家平台的佛事广告位", 0, 2000, 0);
      return false;
    }
  ...
  onClickLinkPreview: function (e) {
    e.preventDefault();
    var self = this, $tar = $(e.target), id = parseInt($tar.attr("data-id")), url = data.getLinkListData[id].url;
    self.showPreviewModal(url);
  },
  onClickLinkEdit: function (e) {
    e.preventDefault();
    var $tar = $(e.target), id = parseInt($tar.attr("data-id")), $modal = $("#edit-link-modal"), $sceneSelect = $modal.find("[data-ele=\"scene-select\"]"), $title = $modal.find("[data-ele=\"link-title\"]"), $linkTextarea = $modal.find("[data-ele=\"link-textarea\"]");
  ...
  onClickSummaryPlay: function (e) {
    var $tar = $(e.target), $audio = $("[data-ele=\"summary-audio\"]"), audio = $audio.get(0), $curPlay = $("[data-ele=\"play-or-pause-btn\"][data-status=\"1\"]"), id = parseInt($tar.attr("data-id")), sound = data.getSceneListData[id].sound, curId = parseInt($audio.attr("data-id")), status = parseInt($tar.attr("data-status"));
    if (curId !== id) {
  ...
  onClickSummaryPreview: function (e) {
    e.preventDefault();
    var $curTar = $(e.currentTarget), id = parseInt($curTar.attr("data-id")), templeId = localStorage.getItem("templeid");
    var url = "https://wx.zizaihome.com/vr/devoteIndex?templeId=" + templeId + "&sceneId=" + id;
    Promotion.show({
      link: url
    });
  },
  ...
  onClickSummaryDelete: function (e) {
    e.preventDefault();
    var $curTar = $(e.currentTarget), id = parseInt($curTar.attr("data-id"));
    $.confirm({
      title: false,
      content: "确定要删除该简介吗？",
      buttons: {
  ...
  showPreviewModal: function (url) {
    var $modal = $("#preview-modal"), $qrcodeContainer = $("#qrcode-container");
