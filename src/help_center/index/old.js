...
var View = Backbone.View.extend({
  ...
  initialize: function (e) {
    ...
    if (typeof self.skip_id != "undefined") {
      ...
      $(".body-inner").eq(0).animate({
        scrollTop: distance
      }, 100);
    }
    if (self.isMobile == 1) {
      $(".book-body").eq(0).bind("scroll", function () {
        var afterTop = $(".book-body").eq(0).scrollTop();
        if (afterTop != 0 && afterTop > self.beforeTop) {
          setTimeout(self.renderPictures(), 100);
        }
      });
    } else {
      $(".body-inner").eq(0).bind("scroll", function () {
        var afterTop = $(".body-inner").eq(0).scrollTop();
        if (afterTop != 0 && afterTop > self.beforeTop) {
          setTimeout(self.renderPictures(), 100);
        }
      });
    }
    self.articleTitle = $("#book-search-results > h1").eq(0).text();
    var articleText = $("#book-search-results > p").text().substring(0, 100), firstSentenceStopIndex = articleText.indexOf("。") + 1;
    if (firstSentenceStopIndex == -1) {
      self.articleDescription = articleText;
    } else {
      self.articleDescription = articleText.substring(0, firstSentenceStopIndex);
    }
    wx.ready(function () {
      wx.onMenuShareTimeline({
        title: self.articleTitle,
        link: self.articleDescription,
        imgUrl: "http://www.zizaihome.cn/icon/temple_128px_1162857_easyicon.net.ico",
        success: function () {},
        cancel: function () {}
      });
  ...
  renderPictures: function (e) {
    var self = this, getContentPics = $("#book-search-results").find("img"), bodyHeight = document.body.offsetHeight;
    if (self.isMobile == 0) {
      var bodyTop = $(".body-inner").eq(0).scrollTop();
    } else {
      var bodyTop = $(".book-body").eq(0).scrollTop();
    }
    ...
    $.each(getContentPics, function (index, ele) {
      ...
      if (distance < bodyHeight / 2) {
        ...
        if (typeof pic_src != "undefined") {
          $(ele).attr("src", pic_src).removeAttr("data-src");
        }
      }
    });
  },
  ...
  changeContent: function (e) {
    ...
    if (typeof self.content_id == "undefined" || self.content_id == "") {
      var $tar = $(".articles").eq(0).find("li").eq(0).find("a").eq(0), cur_content_id = $tar.attr("data-id");
    } else {
      var $tar = $(".sub_chapter").find("[data-id=" + self.content_id + "]").eq(0), cur_content_id = self.content_id;
    }
    $tar.parent().addClass("active");
    var $contentList = $tar.parents(".articles").find("a"), contentIdsArr = [], contentIds = "";
    ...
    $.ajax({
      type: "get",
      url: "/zzhadmin/helpContent/?contentId=" + cur_content_id + "&ids=" + contentIds,
      async: false,
      success: function (res) {
...
var contentView = Backbone.View.extend({
  ...
  initialize: function () {
    this.model.on("change:addTime change:content change:id change:ids change:title", this.render, this);
  },
  render: function () {
