...
$.seeView({
  ...
  onClickShowDialog: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-show-dialog"));
    $("[data-dialog=\"" + id + "\"]").show();
  ...
  onClickSave: function (e) {
    var self = this, $this = $(e.target), handling = $this.attr("data-handling") && !!parseInt($this.attr("data-handling")), result = self.checkFields();
    if (handling) return;
    if (!result.success) {
    ...
    $.seeAjax.post(data.isEdit ? "update" : "add", result, function (res) {
      ...
      if (res.success) {
        commonFunc.alert((data.isEdit ? "保存更新" : "提交审核") + "成功，请耐心等待", function () {
  ...
  checkFields: function () {
    ...
    var values = {
      ...
      licenceImage: !!parseInt($imageLicence.attr("data-uploaded")) ? $imageLicence.attr("src") : "",
      templateImage: !!parseInt($imageTemplate.attr("data-uploaded")) ? $imageTemplate.attr("src") : "",
      idCardImage1: !!parseInt($imageIdCard1.attr("data-uploaded")) ? $imageIdCard1.attr("src") : "",
      idCardImage2: !!parseInt($imageIdCard2.attr("data-uploaded")) ? $imageIdCard2.attr("src") : ""
    };
    if (!values.bank) {
      values.success = !1;
  ...
  onClickPrevStep: function (e) {
    var self = this, result = self.checkFields();
    delete result.success;
    delete result.message;
  ...
  onChangeInputBank: function (e) {
    ...
    if (value === "中国农业银行") $region.removeClass("dp-none"); else $region.addClass("dp-none");
  }
});
