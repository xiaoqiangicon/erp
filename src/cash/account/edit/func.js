...
func.fillInfo = function (info) {
  ...
  if (data.type == 2) {
    info.templateImage && $("#image-template").attr({
      src: info.templateImage,
      "data-uploaded": 1
    });
    info.idCardImage1 && $("#image-id-card-1").attr({
      src: info.idCardImage1,
      "data-uploaded": 1
    });
    info.idCardImage2 && $("#image-id-card-2").attr({
      src: info.idCardImage2,
      "data-uploaded": 1
    });
  } else {
    info.licenceImage && $("#image-licence").attr({
...
func.initUI = function () {
  ...
  !data.isEdit ? $("#prev-step").removeClass("hide") : $("#save").text("保存更新");
};
export default func;
