...
$.seeView({
  ...
  onClickSelectYear: function (e) {
    var $this = $(e.target), year = parseInt($this.attr("data-select-year")), currentYear = parseInt($("[data-selected-year]").attr("data-selected-year")), $yearContainers = $("[data-year-content]"), $targetYearContainer = $("[data-year-content=\"" + year + "\"]"), $paginations = $("[data-pagination]"), $targetPagination = $("[data-pagination=\"" + year + "\"]"), $paginationContentContainers = $("#pagination-content-containers"), $paginationContainers = $("#pagination-containers");
    var currentPage;
    ...
    if (!$targetYearContainer.length) {
      $targetYearContainer = $(commonData.tpl.yearContentContainer.render({
        year: year
      }));
      ...
      $targetYearContainer.html(commonData.tpl.paginationContentContainer.render({
        page: 1,
        year: year
      }));
      commonData.requestList(year, 1);
    } else {
      currentPage = parseInt($("[data-page-index][data-year=\"" + year + "\"].active").attr("data-page-index"));
  ...
  onClickPageIndex: function (e) {
    var $this = $(e.target), year = parseInt($this.attr("data-year")), page = parseInt($this.attr("data-page-index")), currentPage = parseInt($this.attr("data-current-page")), totalPages = parseInt($this.attr("data-total-pages")), $pagination = $("[data-pagination=\"" + year + "\"]"), $paginationContents = $("[data-pagination-content][data-year=\"" + year + "\"]"), $currentYearContent = $("[data-year-content=\"" + year + "\"]"), $targetPaginationContent = $("[data-pagination-content=\"" + page + "\"][data-year=\"" + year + "\"]");
