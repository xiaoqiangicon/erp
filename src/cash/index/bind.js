...
var chartMonths = ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"];
$.seeBind.bind("chart", "#chart-section-canvas", function ($el, data) {
  ...
  var total = _.reduce(data.months, function (memo, num) {
    return memo + num;
  }, 0), titleText = data.year + "年筹集善款  ¥" + total.toFixed(2);
  if (!chart) {
    chartConfig = {
      type: "line",
      data: {
...
$.seeBind.bind("pagination-content", "[data-pagination-content=\"{{page}}\"][data-year=\"{{year}}\"]", function ($el, data) {
  var htmlString = "";
