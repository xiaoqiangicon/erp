...
$.seeView({
  ...
  onClickDialogReceiptsOk: function (e) {
    var self = this, $images = $("[data-popup-receipts-row]");
    if (!$images.length) {
    ...
    fn.confirm("请确认您上传的收据照片是否正确，确认之后则不能更改", function () {
      $.seeAjax.post("addReceipts", {
  ...
  afterSave: function (images) {
    ...
    $("[data-unit-type][data-id=\"" + data.currentHandleId + "\"]").removeClass("red").text(item.typeText);
    $("[data-unit-upload-receipts=\"" + data.currentHandleId + "\"]").removeAttr("data-unit-upload-receipts").removeAttr("data-receipts").attr({
