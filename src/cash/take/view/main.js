...
$.seeView({
  ...
  onChangeSelectDate: function (e) {
    var self = this, $this = $(e.target), $startDate = $("[data-select-date=\"1\"]"), $endDate = $("[data-select-date=\"2\"]"), startDate = $startDate.val(), startDateInt = !!startDate && util.getDateInt(startDate), endDate = $endDate.val(), endDateInt = !!endDate && util.getDateInt(endDate);
  ...
  onClickSpecialDate: function (e) {
    var self = this, $this = $(e.target), type = parseInt($this.attr("data-special-date")), endDate = commonVars.today.display, startDate = util.getDateStartFromToday(specialDateIntervals[type - 1]), status = parseInt($("[data-select-status].active").attr("data-select-status"));
