...
var postHandleOuter = {
  ...
  list: function (res) {
    ...
    if (res.cnt <= 0 && listTotalPages <= 0) {
      ...
      res.pageNumber < 0 ? res.currentPage = listTotalPages : res.currentPage = res.pageNumber + 1;
    } else {
...
$.seeAjax.config({
  ...
  url: {
    list: ["/zzhadmin/getTransactionRecordList/", "/src/cash/record/mock/list_server.json", "/src/cash/record/mock/list.json"]
  },
  requestKeys: {
