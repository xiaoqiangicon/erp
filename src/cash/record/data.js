...
data.getExtremeDate = function (year, month) {
  var date = new Date(year, month, 0), lastDay = date.getDate(), m = ("" + month).length > 1 ? "" + month : "0" + month;
  return {
    start: "" + year + "-" + m + "-" + "01",
    end: "" + year + "-" + m + "-" + lastDay
  };
...
data.getDateInt = function (dateStr) {
  return parseInt(dateStr.slice(0, 4) + dateStr.slice(5, 7) + dateStr.slice(8, 10));
};
data.getDateStartFromToday = function (intervalMonths) {
  var year = data.today.year, month = data.today.month, day = data.today.day, intervalYear = 0, targetLastDay;
  !intervalMonths && (intervalMonths = 0);
  month -= intervalMonths;
  month <= 0 && (intervalYear = Math.abs(Math.floor((month - 1) / 12)), year -= intervalYear, month += intervalYear * 12);
