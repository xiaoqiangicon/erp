...
(function () {
  var i, il, currentYear = commonVars.today.year, $selectYearContent1 = $("[data-select-year-content=\"1\"]"), $selectYearContent2 = $("[data-select-year-content=\"2\"]"), $statusContainer1 = $("[data-status-container=\"1\"]"), $statusContainer2 = $("[data-status-container=\"2\"]");
  for ((i = currentYear, il = 2016); i >= il; i--) {
    ($selectYearContent1.append(tpl.selectYearItem.render({
      year: i,
      status: 1
    })), $selectYearContent2.append(tpl.selectYearItem.render({
      year: i,
      status: 2
    })));
  }
  ...
  $statusContainer1.append(tpl.yearContentContainer.render({
    year: currentYear,
    status: 1
  }));
  $statusContainer2.append(tpl.yearContentContainer.render({
    year: currentYear,
    status: 2
  }));
...
$sameHeightEls.map(function () {
  ...
  if (!sameHeightRecord[key] || height > sameHeightRecord[key]) sameHeightRecord[key] = height;
});
$sameHeightEls.map(function () {
  var $this = $(this);
  var key = $this.attr("data-same-height");
