...
// 增加remote中sendType和stringify功能 2017/4/24
// 增加对toast的依赖，在无网络状态下提示信息 2017/6/19
// 增加updatePaginationWidth 更新底部分页宽度
!function ($) {
    "use strict";
    ...
    var Page = function (element, options) {
        var defaultOption = {
            ...
            nextBtnText: "&raquo;",
            loadFirstPage: true,
            paginationName: '.content-pagination-container', //新增的分页名称，如果一个页面有多个pagination传进来控制宽度
            remote: {
                url: null,
                sendType: 'GET',
                stringify: !1,
        ...
        this.paginationName = this.options.paginationName;
        ...
        var init = function (obj) {
            ...
            //init size module
            var pre_html = $('<span class="pagination-size-info mgr5">每页</span>');
            var after_html = $('<span class="pagination-size-info mgl5">条</span>');
            ...
            that.$size.append(pre_html).append(html).append(after_html);
        ...
        var eventHandler = function (event) {
            ...
            if (event.type === 'click' && $target.data('pageIndex') !== undefined && !$target.parent().hasClass('active')) {
                ...
                var pageSize = parseInt(that.$size.find('select').val());
    ...
    Page.prototype = {
        ...
        remote: function (pageIndex, params) {
            ...
            var requestParams = $.extend({}, this.options.remote.params, pageParams),
                sendType = '';
            this.options.remote.stringify ? (requestParams = JSON.stringify(requestParams)) : (requestParams = requestParams);
            this.options.remote.sendType ? (sendType = this.options.remote.sendType) : (sendType = 'GET');
            $.ajax({
        ...
        renderPagination: function (pageIndex) {
            ...
            this.updatePaginationWidth(this.paginationName, pages.length);
        },
        updatePaginationWidth: function (paginationFooter, pagesLen) {
            var arrLen = [];
            // arrLen[0] = $(paginationFooter + " .m-pagination-page").attr("style") == "display: none;" ? 0 : $(paginationFooter + " .m-pagination-page").outerWidth(true);
            arrLen[0] = pagesLen * 35 + 70;
            arrLen[1] = $(paginationFooter + " .m-pagination-size").attr("style") == "display: none;" ? 0 : $(paginationFooter + " .m-pagination-size").outerWidth(true);
            arrLen[2] = $(paginationFooter + " .m-pagination-jump").attr("style") == "display: none;" ? 0 : $(paginationFooter + " .m-pagination-jump").outerWidth(true);
            arrLen[3] = $(paginationFooter + " .m-pagination-info").attr("style") == "display: none;" ? 0 : $(paginationFooter + " .m-pagination-info").outerWidth(true);
