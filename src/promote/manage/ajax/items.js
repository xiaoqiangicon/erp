  ...
  res.data.forEach(item => {
    ...
    if (!item.hasCharge || item.priceType === 2) item.chargeText = '-';
      ...
      if (item.priceType === 3) item.chargeText = `支付价格*${item.charge}%`;
        ...
        item.chargeText = parseFloat(
          ((item.price * item.charge) / 100).toFixed(2)
        );
    }
    ...
    if (!item.hasReward || item.priceType === 2) item.rewardText = '-';
      ...
      if (item.priceType === 3) item.rewardText = `支付价格*${item.reward}%`;
        ...
        item.rewardText = parseFloat(
          ((item.price * item.reward) / 100).toFixed(2)
        );
    }

    if (item.promote)
      item.promote = parseFloat((item.promote * 100).toFixed(2));
...
const postLocal = res => {
  res.data.forEach(item => {
    ...
    if (!item.hasCharge) item.chargeText = '-';
      ...
      item.chargeText = `支付价格*${item.charge}%`;
...
seeAjax.config('items', {
  url: [
    ...
    '/src/promote/manage/mock/items-1',
    '/src/promote/manage/mock/items',
  ],
  pre: [pre, pre],
