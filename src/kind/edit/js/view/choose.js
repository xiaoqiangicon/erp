      ...
      coverChoose = new ChooseImage({
        onSubmit: items => {
          ...
          items.forEach((item, index) => {
            index < remainLength && $coverContainer.append(coverItemTpl({
              image: item.src
            }));
          });
    ...
    if (!shareChoose) {
      shareChoose = new ChooseIcon({
        ...
        onSubmit: items => {
          ...
          $shareIconContainer.append(shareItemTpl({
            image: items[0].src
          }));
  ...
  onClickPayItemAdd: e => {
    ...
    if (!payChoose) {
      payChoose = new ChooseImage({
        multiSelect: !1,
        onSubmit: items => {
          const $container = $currentPayItemAdd.parents("[data-pay-item-icon-container]");
