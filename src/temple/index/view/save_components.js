...
$.seeView({
  ...
  onClickSaveComponent: function (e) {
    var self = this, $this = $(e.target), id = parseInt($this.attr("data-id")), type = parseInt($this.attr("data-type")), $displayComponent = $("[data-container=\"component-display\"][data-type=\"" + type + "\"][data-id=\"" + id + "\"]"), isUpdate = !!parseInt($displayComponent.attr("data-is-update")), sortId = parseInt($displayComponent.attr("data-server-sort-id")), componentIndex = 0;
    var $allDisplayComponents = $("[data-container=\"component-display\"]");
    $allDisplayComponents.map(function (index) {
      var $this = $(this), thisType = parseInt($this.attr("data-type")), thisId = parseInt($this.attr("data-id"));
      thisId === id && thisType === type && (componentIndex = index + 1);
    });
    if (!self.checkComponentCompleteness(id, type, componentIndex, isUpdate, sortId)) return !1;
    currentSaveId = id;
    currentSaveType = type;
    currentSaveIsUpdate = isUpdate;
    self.preHandleComponentsData();
    confirm("即将保存您正在操作的组件，保存成功之后将会刷新当前的页面，您其他还未保存的组件数据将会丢失。确认保存这个组件吗？", function () {
      self.save();
    });
  },
  save: function () {
  ...
  checkComponentCompleteness: function (id, type, index, isUpdate, sortId) {
    ...
    switch (type) {
      case 1:
        return self.checkComponentCompletenessForIntroduction(id, index, isUpdate, sortId);
        break;
      case 2:
        return self.checkComponentCompletenessForPersonSaying(id, index, isUpdate, sortId);
        break;
      case 3:
        return self.checkComponentCompletenessForSwipeList(id, index, isUpdate, sortId);
        break;
      case 4:
        return self.checkComponentCompletenessForDonateChart(id, index, isUpdate, sortId);
        break;
      case 5:
        return self.checkComponentCompletenessForCalendar(id, index, isUpdate, sortId);
        break;
      case 6:
        return self.checkComponentCompletenessForShortcut(id, index, isUpdate, sortId);
        break;
      case 7:
        return self.checkComponentCompletenessForHouse(id, index, isUpdate, sortId);
        break;
      default:
        break;
    }
  },
  checkComponentCompletenessForIntroduction: function (id, index, isUpdate, sortId) {
    var $imageContainers, images = [], $provinceSelect, province, $citySelect, city, $districtSelect, district, $introductionTextarea, introduction;
    var $componentContainer = $("[data-container=\"component-display\"][data-type=\"1\"][data-id=\"" + id + "\"]");
    $imageContainers = $("[data-upload-image-container=\"" + id + "\"][data-type=\"1\"]");
    ...
    $imageContainers.map(function (index) {
      var $this = $(this), id = parseInt($this.attr("data-image-id")), isUpdate = parseInt($this.attr("data-is-update")), imageUrl = $this.find("img").attr("src"), markIndex = imageUrl.indexOf("?");
      images.push({
        id: isUpdate ? id : 0,
  ...
  checkComponentCompletenessForPersonSaying: function (id, index, isUpdate, sortId) {
    var $componentContainer = $("[data-container=\"component-display\"][data-type=\"2\"][data-id=\"" + id + "\"]");
    if (!share.figureComponent.components || !share.figureComponent.components.length) {
      $.alert({
        title: false,
        content: "请至少添加一个高僧法师",
        buttons: {
  ...
  checkComponentCompletenessForSwipeList: function (id, index, isUpdate, sortId) {
    var self = this, $title, title, $displaySource, displaySource, $sourceCategory, sourceCategory, $buddhistCategory, buddhistCategory, $articleCategory, articleCategory, $itemsCount, itemsCount, $subType, subType, $showTitle, showTitle, $showMore, showMore, $imageContainers, images = [];
    var $componentContainer = $("[data-container=\"component-display\"][data-type=\"3\"][data-id=\"" + id + "\"]");
    ...
    $displaySource = $("[data-swipe-list-display-source=\"" + id + "\" ][data-type=\"" + sourceCategory + "\"]:checked");
    displaySource = parseInt($displaySource.val());
    $buddhistCategory = $("[ data-swipe-list-source-sub-category=\"" + id + "\"][data-type=\"1\"]");
    buddhistCategory = parseInt($buddhistCategory.val());
    $articleCategory = $("[ data-swipe-list-source-sub-category=\"" + id + "\"][data-type=\"2\"]");
    articleCategory = parseInt($articleCategory.val());
    $itemsCount = $("[data-swipe-list-items-count=\"" + id + "\"]");
    itemsCount = parseInt($itemsCount.val());
    $imageContainers = $("[data-upload-image-container][data-type=\"3\"]" + "[data-source-type=\"" + sourceCategory + "\"][data-component-id=\"" + id + "\"]");
    ...
    $imageContainers.map(function (index) {
      var $this = $(this), id = parseInt($this.attr("data-upload-image-container")), imageUrl = $this.attr("data-image"), markIndex = imageUrl.indexOf("?");
      images.push({
  ...
  checkComponentCompletenessForDonateChart: function (id, index, isUpdate, sortId) {
    var self = this, $title, title, $itemsCount, itemsCount, $realTimeCheckbox = $("[data-edit-donate-chart-type][data-index=\"1\"]"), realTimeActive = $realTimeCheckbox.prop("checked"), $monthCheckbox = $("[data-edit-donate-chart-type][data-index=\"2\"]"), monthActive = $monthCheckbox.prop("checked"), $totalCheckbox = $("[data-edit-donate-chart-type][data-index=\"3\"]"), totalActive = $totalCheckbox.prop("checked");
    var $componentContainer = $("[data-container=\"component-display\"][data-type=\"4\"][data-id=\"" + id + "\"]");
  ...
  checkComponentCompletenessForCalendar: function (id, index, isUpdate, sortId) {
    var self = this, data = {}, $dateCells = $("[data-edit-calendar-new-add-cell]"), $activitiesCell;
    $dateCells.map(function () {
      var $this = $(this), year = $this.attr("data-year"), month = $this.attr("data-month"), day = $this.attr("data-day"), currentData = [];
      $activitiesCell = $this.find("[data-edit-calendar-new-add-activity-cell]");
      $activitiesCell.map(function () {
        var $this = $(this), id = parseInt($this.attr("data-id")), type = parseInt($this.attr("data-type"));
        currentData.push({
          id: id,
          type: type
        });
      ...
      month += "";
      day += "";
      data[year + "-" + (month.length <= 1 ? "0" + month : month) + "-" + (day.length <= 1 ? "0" + day : day)] = currentData;
    });
    componentData = {
      id: isUpdate ? id : 0,
      type: 5,
  ...
  checkComponentCompletenessForShortcut: function (id, index, isUpdate, sortId) {
    var self = this, $title = $("[data-edit-shortcut-title=\"" + id + "\"]"), title = $title.val().trim(), $items = $("[data-edit-shortcut-cell=\"" + id + "\"]"), items = [];
    var $componentContainer = $("[data-container=\"component-display\"][data-type=\"6\"][data-id=\"" + id + "\"]");
    if (!$items.length) {
      $.alert({
        title: false,
  ...
  checkComponentCompletenessForHouse: function (id, index, isUpdate, sortId) {
    var $componentContainer = $("[data-container=\"component-display\"][data-type=\"7\"][data-id=\"" + id + "\"]");
    ...
    houses.forEach(function (i) {
      var covers = i.covers.map(function (cover) {
        ...
        if (avatarMarkIndex >= 0) return cover.slice(0, avatarMarkIndex); else return cover;
      });
      ...
      if (componentData.type === 2) {
        ...
        Object.keys(componentData.data).map(function (date) {
          componentData.data[date].map(function (activity) {
            ...
            activity.type === 1 ? data.commodityId = activity.id : data.title = indexData.calendarNewAddedTitle[activity.id];
