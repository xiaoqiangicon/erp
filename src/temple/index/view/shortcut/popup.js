...
$.seeView({
  events: {
    ...
    "click [data-edit-shortcut-popup-image-cell-delete]": "onClickPopupImageCellDelete",
  ...
  onClickShortcutPopup: function (e) {
    var $this = $(e.target), $modal = $this.parents(".modal"), close = !!$this.attr("data-popup-close") || !!$this.attr("data-popup-backdrop") || !!$this.parent().attr("data-popup-close");
    !!close && $modal.hide();
  },
  onClickPopupAddBtn: function (e) {
    var self = this, $this = $(e.target), id = parseInt($this.attr("data-edit-shortcut-popup-add-btn")), chooseIcon = chooseIconInstances[id];
    if (!chooseIcon) {
  ...
  afterGetImage: function (data, id) {
    var $image = $("[data-edit-shortcut-popup-image-cell=\"" + id + "\"]"), $addEl = $("[data-edit-shortcut-popup-add=\"" + id + "\"]");
    if (!data.length) {
      return;
    }
    (!$image.length ? ($image = $(shortcutTpl.editPopupImageCell.render({
      id: id,
      image: data[0].src
    })), $addEl.before($image)) : ($image.attr({
  ...
  onClickPopupImageCellDelete: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-edit-shortcut-popup-image-cell-delete"));
    $("[data-edit-shortcut-popup-image-cell=\"" + id + "\"]").remove();
    $("[data-edit-shortcut-popup-add=\"" + id + "\"]").show();
  },
  onChangePopupType: function (e) {
    var $tempThis = $(e.target), id = parseInt($tempThis.attr("data-edit-shortcut-popup-type")), $this = $("[data-edit-shortcut-popup-type=\"" + id + "\"]:checked"), value = parseInt($this.attr("value"));
    $("[data-edit-shortcut-popup-type-content=\"" + id + "\"]").hide();
    $("[data-edit-shortcut-popup-type-content=\"" + id + "\"][data-value=\"" + value + "\"]").show();
    if (!shortcutViewUtil.inRenderEditItem) {
      var selectedType = parseInt($("[data-shortcut-popup-selected-type=\"" + id + "\"]").attr("data-type"));
      ...
      if (value == 2 && Object.keys(defaultMatches).indexOf(selectedType + "") > -1) {
        $title.val(defaultMatches[selectedType].name);
        if (!$image.length) {
          $add.before(shortcutTpl.editPopupImageCell.render({
            id: id,
  ...
  onClickPopupSelectType: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-shortcut-popup-select-type")), type = parseInt($this.attr("data-type")), url = $this.attr("data-url"), hasSubType = parseInt($this.attr("data-has-sub-type")), name = $this.text();
    var $selected = $("[data-shortcut-popup-selected-type=\"" + id + "\"]"), selectedType = parseInt($selected.attr("data-type"));
    ...
    $("[data-shortcut-popup-sub-type-container=\"" + id + "\"][data-parent-type=\"" + type + "\"]").show();
    if (!shortcutViewUtil.inRenderEditItem) {
      var $title = $("[data-edit-shortcut-popup-name=\"" + id + "\"]");
      var $image = $("[data-edit-shortcut-popup-image-cell=\"" + id + "\"]");
      var $add = $("[data-edit-shortcut-popup-add=\"" + id + "\"]");
  ...
  onClickPopupSelectSubType: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-shortcut-popup-select-sub-type")), parentType = parseInt($this.attr("data-parent-type")), subTypeId = parseInt($this.attr("data-sub-type-id")), url = $this.attr("data-url"), name = $this.text();
    var $selected = $("[data-shortcut-popup-selected-sub-type=\"" + id + "\"][data-parent-type=\"" + parentType + "\"]"), selectedId = parseInt($selected.attr("data-sub-type-id"));
  ...
  onClickPopupConfirm: function (e) {
    var self = this, $this = $(e.target), id = parseInt($this.attr("data-edit-shortcut-popup-confirm"));
  ...
  checkPopupData: function (id) {
    ...
    var $name = $("[data-edit-shortcut-popup-name=\"" + id + "\"]"), name = $name.val().trim();
    if (!name) {
      data.message = "名称不能为空，请输入后再保存";
      return data;
    }
    data.data.title = name;
    var linkType = parseInt($("[data-edit-shortcut-popup-type=\"" + id + "\"]:checked").attr("value"));
    data.data.linkType = linkType;
    if (linkType == 2) {
      var $type = $("[data-shortcut-popup-selected-type=\"" + id + "\"]"), type = parseInt($type.attr("data-type")), hasSubType = parseInt($type.attr("data-has-sub-type")), $subType = hasSubType && $("[data-shortcut-popup-selected-sub-type=\"" + id + "\"][data-parent-type=\"" + type + "\"]"), subTypeId = hasSubType && parseInt($subType.attr("data-sub-type-id"));
      data.data.type = type;
      data.data.subTypeId = hasSubType ? subTypeId : 0;
      data.data.link = !hasSubType ? $type.attr("data-url") : $subType.attr("data-url");
    } else {
      var $link = $("[data-edit-shortcut-popup-link=\"" + id + "\"]"), link = $link.val().trim();
      if (!link) {
  ...
  afterGetItem: function (data, id) {
    ...
    if (!shortcutViewUtil.currentEditSequence) {
      var $addEl = $("[data-edit-shortcut-add=\"" + id + "\"]"), $displayBody = $("[data-display-shortcut-body=\"" + id + "\"]"), existedItemsLength = $("[data-edit-shortcut-cell=\"" + id + "\"]").length;
      ...
      if (existedItemsLength >= indexData.misc.shortcutItemsLimit - 1) $addEl.hide();
    } else {
      var $editEl = $($("[data-edit-shortcut-cell=\"" + id + "\"]")[shortcutViewUtil.currentEditSequence - 1]);
      var $displayEl = $($("[data-display-shortcut-cell=\"" + id + "\"]")[shortcutViewUtil.currentEditSequence - 1]);
      $editEl.attr({
