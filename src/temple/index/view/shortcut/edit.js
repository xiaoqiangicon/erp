...
$.seeView({
  ...
  onKeyupEditShortcutTitle: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-edit-shortcut-title")), $title = $("[data-display-shortcut-title=\"" + id + "\"]"), value = $this.val().trim();
    $title.text(value);
    !value ? $title.hide() : $title.show();
  },
  onClickEditShortcutAddBtn: function (e) {
    var self = this, $this = $(e.target), id = parseInt($this.attr("data-edit-shortcut-add-btn")), $popup = $("[data-shortcut-popup=\"" + id + "\"]");
    if (!!$popup.length) {
      $popup.remove();
    }
    $popup = $(shortcutTpl.editPopup.render({
      id: id,
  ...
  onClickCellDelete: function (e) {
    var self = this, $this = $(e.target), id = parseInt($this.attr("data-edit-shortcut-cell-delete"));
  ...
  onClickCellEdit: function (e) {
    var self = this, $this = $(e.target), id = parseInt($this.attr("data-edit-shortcut-cell-edit")), $parent = $this.parent(), sequence = 0, $popup = $("[data-shortcut-popup=\"" + id + "\"]");
    $("[data-edit-shortcut-cell=\"" + id + "\"]").map(function (index) {
      if (this == $parent[0]) sequence = index + 1;
    });
    var title = $parent.attr("data-title"), image = $parent.attr("data-image"), link = $parent.attr("data-link"), type = parseInt($parent.attr("data-type")), linkType = parseInt($parent.attr("data-link-type")), subTypeId = parseInt($parent.attr("data-sub-type-id"));
    if (!$popup.length) {
      $popup = $(shortcutTpl.editPopup.render({
        id: id,
        appTypes: indexData.misc.appTypes
  ...
  renderShortcutPopup: function (editData) {
    ...
    $("[data-edit-shortcut-popup-name=\"" + editData.id + "\"]").val(editData.title);
    var $addEl = $("[data-edit-shortcut-popup-add=\"" + editData.id + "\"]");
    var $image = $("[data-edit-shortcut-popup-image-cell=\"" + editData.id + "\"]");
    if ($image.length) {
      $image.attr({
      ...
      $image = $(shortcutTpl.editPopupImageCell.render({
        id: editData.id,
    ...
    $addEl.hide();
    $("[data-edit-shortcut-popup-type=\"" + editData.id + "\"][value=\"" + editData.linkType + "\"]").trigger("click");
    if (editData.linkType == 1) {
      $("[data-edit-shortcut-popup-link=\"" + editData.id + "\"]").val(editData.link);
    } else {
      var $selectType = $("[data-shortcut-popup-select-type=\"" + editData.id + "\"][data-type=\"" + editData.type + "\"]");
      var $selectedType = $("[data-shortcut-popup-selected-type=\"" + editData.id + "\"]");
      ...
      $("[data-shortcut-popup-sub-type-container=\"" + editData.id + "\"]").hide();
      if (hasSubType) {
        $("[data-shortcut-popup-sub-type-container=\"" + editData.id + "\"][data-parent-type=\"" + editData.type + "\"]").show();
        var $selectSubType = $("[data-shortcut-popup-select-sub-type=\"" + editData.id + "\"][data-parent-type=\"" + editData.type + "\"][data-sub-type-id=\"" + editData.subTypeId + "\"]");
        var $selectedSubType = $("[data-shortcut-popup-selected-sub-type=\"" + editData.id + "\"][data-parent-type=\"" + editData.type + "\"]");
        $selectedSubType.attr({
