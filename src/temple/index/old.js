...
$("#components-display-container").sortable({
  stop: function () {
    var $editContainerParent = $("#design-sidebar"), $activeDisplayComponent = $("[data-container=\"component-display\"].active");
...
$.seeAjax.get("buddhistTypes", {}, function (res) {
  res.success ? (indexData.misc.buddhistTypes = res.data, handleAfterLoadTypes()) : !!res.message && console.error("获取佛事列表失败：" + res.message);
});
$.seeAjax.get("articleTypes", {}, function (res) {
  res.success ? (indexData.misc.articleTypes = res.data, handleAfterLoadTypes()) : !!res.message && console.error("获取文章列表失败：" + res.message);
});
$.seeAjax.get("appTypes", {}, function (res) {
  res.success ? indexData.misc.appTypes = res.data : !!res.message && console.error("获取应用列表失败：" + res.message);
});
function handleAfterLoadTypes() {
  typesLoadedCount += 1;
  if (typesLoadedCount < 2) return;
  $.seeAjax.get("savedData", {}, function (res) {
    res.success && (renderSavedData(res), !!res.data && !!res.data.length && $("#fixed-bottom").show());
    $("#loading-toast").hide().removeClass("unloaded");
  });
}
