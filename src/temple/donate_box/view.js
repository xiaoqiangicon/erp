...
$.seeView({
  ...
  onClickFilterSubmit: function (e) {
    var $startDate = $("#filter-start-date"), startDate = $startDate.val(), startDateInt = !!startDate && parseInt(startDate.slice(0, 4) + startDate.slice(5, 7) + startDate.slice(8, 10)), $endDate = $("#filter-end-date"), endDate = $endDate.val(), endDateInt = !!endDate && parseInt(endDate.slice(0, 4) + endDate.slice(5, 7) + endDate.slice(8, 10));
  ...
  onClickPromotion: function (e) {
    ...
    if (!$promotionPopup.length) {
      $promotionPopup = $(tpl.promotionPopup.render({
        link: data.shareLink
      }));
  ...
  onClickSwitchQrcode: function (e) {
    var $this = $(e.target), type = parseInt($this.attr("data-type"));
  ...
  onClickExport: function (e) {
    ...
    window.open("/zzhadmin/meritBoxDownloadExcel/?startTime=" + startDate + "&endTime=" + endDate);
  }
});
