...
var foshiId = null, templateId = null, editType = null, foshiName = null, ifTemplate = null, verifyId = null, sizeIteration = 0, origin_printer_list = [], printer_list = [], selection_list = [], no_selection_list = [], ifFirstSave = true, first_set = true, overall_if_open_printer = false, set_imagetext = 1, template_content = {}, has_local_storage = false, has_tip = false, if_set_printer = false, if_submiting_pic = false, ifSingleMoney;
var View = Backbone.View.extend({
  ...
  events: {
    ...
    "click [data-action=\"close\"][data-type=\"promotion\"]": "onClickClosePromotion",
    "click #sureCalendar": "sureCalendar"
  },
  onKeyupSTextarea: function (e) {
    var $curTar = $(e.currentTarget), $numTip = $curTar.next(), textLen = $curTar.val().length, maxLen = $curTar.prop("maxlength");
    $numTip.html(textLen + "/" + maxLen);
  },
  showClassificationModal: function () {
    var self = this, classification = self.res, fragmentHtml = "";
    _.each(classification, function (item, index) {
      fragmentHtml += "<li class='list-group-item' style='overflow: hidden;'>" + "<input  data-id=" + item.ceremonyTypeId + " class='form-control pull-left ' style='width: 200px' value=" + item.name + "  />" + "<button class='btn btn-warning btn-xs pull-right deleteClassificationList' data-id=" + item.ceremonyTypeId + "  style='margin-top: 6px;'>删除</button>" + "<button class='btn btn-warning btn-xs pull-right reviseClassificationList' data-id=" + item.ceremonyTypeId + "  style='margin-top: 6px; margin-right:6px;'>修改</button>" + "</li>";
    });
    $("#classificationSelBox").html(fragmentHtml);
  },
  hideClassificationModal: function (e) {
    var self = this, classification = self.res, currentSelect = $("#classification").selectpicker("val");
    ...
    if (typeof classification == "object" && classification.length > 0) {
      ...
      var opHtml = "";
      _.each(classification, function (item, index) {
        opHtml += "<option value=" + item.ceremonyTypeId + " data-id=" + item.ceremonyTypeId + ">" + item.name + "</option>";
      });
  ...
  deleteClassificationList: function (e) {
    var self = this, $tar = $(e.target), classification = self.res, id = parseInt($tar.attr("data-id")), typeopt = {};
    typeopt["id"] = id;
    $.confirm({
      title: false,
      content: "你确定删除吗?",
      buttons: {
        ok: {
          text: "确定",
          action: function () {
            doRequest("/zzhadmin/delCeremonyType/", typeopt, function (res) {}, function (msg) {});
  ...
  reviseClassificationList: function (e) {
    var self = this, $tar = $(e.target), classification = self.res, id = parseInt($tar.attr("data-id")), name = $tar.prev().prev().val(), typeopt = {};
    typeopt["id"] = id;
    typeopt["name"] = name;
    doRequest("/zzhadmin/managerEditCeremonyType/", typeopt, function (res) {
      if (res.result == 0) {
        Toast("修改成功!");
      }
    }, function (msg) {});
  ...
  addClassificationList: function (e) {
    var self = this, $tar = $(e.target), classification = self.res, value = $("#addClassificationItem").val();
    if ($.trim(value).length > 0) {
      var typeopt = {};
      typeopt["name"] = value;
      doRequest("/zzhadmin/createCeremonyType/", typeopt, function (res) {
  ...
  uploadCoverPic: function (e) {
    var self = this, $tar = $(e.target), maxAddPics = 5 - $tar.parents("#pictureBox").find("img").length;
      ...
      onSubmit: function (data) {
        if (data.length > maxAddPics) {
          ...
          for (var i = 0; i < data.length; i++) {
            var html = self.html, imgSrc = data[i].src, sizePicId = $tar.attr("id");
            if (data.length == 5) {
              $(".upload_box", "#pictureBox").not(".upload_wrap_change ").addClass("hideEle");
            }
            $(".pic_icon").css("opacity", "0").removeClass("tip_icon_error");
            var img = document.createElement("img");
            img.src = imgSrc;
            var PicsSpan = document.createElement("span");
            PicsSpan.setAttribute("class", "glyphicon glyphicon-remove-circle delPic");
            PicsSpan.setAttribute("style", "position:absolute;right:3px;color:rgb(212, 106, 64);");
            var length = $tar.parents(".upload_box").find("img").not(img).remove().length;
            var modihtml = html.replace(/coverPic0/, "coverPic" + self.PicId++).replace(/data-id="uploadCover"/, "");
            if (length == 0) {
              $(".upload_wrap").prepend(modihtml);
            }
            $("#coverPic" + ++self.PicssId).removeClass("picture").attr("type", "").addClass("dpblock").parents(".upload_box").addClass("upload_wrap_change ui-state-default posRel").append(img).append(PicsSpan);
            $(img).css({
              width: "100px",
              height: "100px",
              position: "absolute",
  ...
  delPic: function (e) {
    var self = this, $tar = $(e.target);
    $tar.parents(".upload_box").remove();
    var picsLen = $(".upload_wrap").find("img").length;
    if (picsLen == 4) {
      $(".upload_box", "#pictureBox").not(".upload_wrap_change ").removeClass("hideEle");
    }
  },
  ...
  generateRandom: function (e) {
    var $tar = $(e.target);
    e.preventDefault();
    var randomList = "5,5,5,5.55,5.88,6,6,6.66,6.68,6.88,7.77,8,8.88,9.66,9.99,10,10,10,10,10.01,10.88,11.11,13.14,13.88,15.68,16,16.66,16.88,18,18.18,18.88,19.19,19.88,20,20,20,20.66,22,22.22,25.55,26.66,26.88,28,28.88,33.33,36,38.88,46.66,49.99,50,66.66,68.68,88.88,99.99,100,108,168";
  ...
  detectSuixiInput: function (e) {
    var $tar = $(e.target);
    var get_val = $tar.val();
    var showRandom = get_val.toString().indexOf(",") + get_val.toString().indexOf("，");
    if (showRandom > -2) {
      ...
      $tar.next().next().removeClass("hide");
    } else {
      $tar.next().removeClass("hide");
      $tar.next().next().addClass("hide");
      $("#autoFinishSwitch").bootstrapSwitch("destroy");
    }
  },
  blurSuixiInput: function (e) {
    var self = this;
    var $tar = $(e.target), get_val = $tar.val();
    if (get_val != "") {
      var suixi_match_right = get_val.toString().match(myRegExp.suixi_price_right);
      if (suixi_match_right) {} else {
  ...
  showSuixiInput: function (e) {
    var $suixiInput = $("#suixiInput").parents(".form-group"), $generateRandom = $("#generateRandom"), suixiInputVal = $("#suixiInput").val(), showRandom = suixiInputVal.indexOf(",") + suixiInputVal.indexOf("，");
    $(e.target).prop("checked") == true ? $suixiInput.removeClass("hide") : $suixiInput.addClass("hide");
    if (showRandom > -2) {
      $generateRandom.addClass("hide");
    } else {
      $(e.target).prop("checked") == true ? $generateRandom.removeClass("hide") : $generateRandom.addClass("hide");
    }
  },
  hideSuixiInput: function (e) {
    var $suixiInput = $("#suixiInput").parents(".form-group"), $generateRandom = $("#generateRandom");
    $(e.target).prop("checked") == true ? $suixiInput.addClass("hide") : $suixiInput.removeClass("hide");
    $(e.target).prop("checked") == true ? $generateRandom.addClass("hide") : $generateRandom.removeClass("hide");
  },
  showSubSetModal: function (e) {
    var $relatedTarget = $(e.relatedTarget), $tar = $(e.target), $subEndTime = $("#sub-end-time"), $subSetModal = $("#sub-set-modal");
    if ($tar.get(0) === $subEndTime.get(0)) {
      return;
    }
    var self = this, cid = $relatedTarget.attr("data-cid"), curSubModal = self.sizes.get(cid), $saveSubSet = $("#save-sub-set"), $subSetModalTitle = $("#sub-set-modal-title"), name = curSubModal.get("name"), endTime = curSubModal.get("endTime"), enrollNum = curSubModal.get("enroll_num"), subType = curSubModal.get("subdivide_type");
    $saveSubSet.attr("data-cid", cid);
    $subSetModalTitle.text("选择项设置：" + name);
    $subEndTime.val(endTime);
    $("[name=\"sub-enroll-limit\"][value=\"" + enrollNum + "\"]").prop("checked", true);
  ...
  onClickSaveSubSet: function (e) {
    var $tar = $(e.target), $modal = $("#sub-set-modal"), self = this, cid = $tar.attr("data-cid"), curSubModal = self.sizes.get(cid), subType = curSubModal.get("subdivide_type"), verifyObj = {
      value: true,
      reason: ""
    };
    var $endTime = $("#sub-end-time"), endTime = $endTime.val();
  ...
  showSizeBox: function (e) {
    var self = this;
    e.preventDefault();
    var sizeBox = $("#proStyle").parents(".form-group"), addBox = $("#proStyleBtn").parents("div").prev()[0];
    addBox.innerHTML = "";
    if (sizeBox.hasClass("hide")) {
      ...
      self.sizesView.render();
      $("#proStore,#needPay").parents(".form-group").hide();
      $("#generateRandom").parents(".form-group").addClass("hide");
  ...
  onChangeSubType: function (e) {
    var self = this, $tar = $(e.target), subType = $tar.val();
    e.preventDefault();
  },
  proName: function (e) {
    var titleLen = $("#proName").val().length;
    if (titleLen > 30) {
      $("#proName").nextAll(".nullError_msg").removeClass("hide");
    } else {
      $("#proName").nextAll(".nullError_msg").addClass("hide");
    }
  },
  setProValue: function (e) {
    var self = this, $tar = $(e.target), value = $tar.val(), cid = $tar.attr("data-cid");
    if ($tar.hasClass("proSizeName")) {
      ...
      if (value != "") {
        var suixi_match_right = value.toString().match(myRegExp.suixi_price_right);
        if (suixi_match_right) {
          if (suixi_match_right[0].indexOf(",") + suixi_match_right[0].indexOf(",") === -2) {
          ...
          self.sizes.get(cid).set("isSingleMoney", 1);
          Toast("选择项支付金额格式出错，请以逗号分隔数字且无其它无关字符出现", 2);
          self.addRedBorder($tar);
        }
      } else {
        self.sizes.get(cid).set("isSingleMoney", 1);
  ...
  showRandomPrice: function (e) {
    var $tar = $(e.target), inputValue = $tar.val().toString();
    e.preventDefault();
    var ifHide = inputValue.indexOf(",") + inputValue.indexOf("，");
    if (ifHide > -2) {
      $tar.prev().prev().removeClass("hide");
      $tar.prev().addClass("hide");
    } else {
  ...
  generateRandomPrice: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.next().attr("data-cid");
    e.preventDefault();
    var randomList = "5,5,5,5.55,5.88,6,6,6.66,6.68,6.88,7.77,8,8.88,9.66,9.99,10,10,10,10,10.01,10.88,11.11,13.14,13.88,15.68,16,16.66,16.88,18,18.18,18.88,19.19,19.88,20,20,20,20.66,22,22.22,25.55,26.66,26.88,28,28.88,33.33,36,38.88,46.66,49.99,50,66.66,68.68,88.88,99.99,100,108,168";
    ...
    if (uploadBox.hasClass("upload_wrap_change")) {} else {
      var img = document.createElement("img");
      if (self.locationBc === 0) {
        img.src = "https://pic.zizaihome.com/e0898a9c-23a1-11e9-9b75-00163e0c001e.jpg";
      ...
      if ($(".rePos").length != 0) {
        var right = -$(".rePos").eq(0).position().left;
  ...
  autoFinishOrder: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.parent().next().next().attr("data-cid");
    $tar.parent().find("input.autoFinishSwitch").bootstrapSwitch("toggleState");
  },
  ...
  detectRandom: function (e) {
    ...
    if (showRandom > -2) {
      ...
      $tar.prev().prev().removeClass("hide");
      if (ifSingleMoney) {
        $tar.prev().prev().find(".autoFinishSwitch").bootstrapSwitch("state", true);
      }
      self.sizes.get(cid).set("isAutoFinish", Number($tar.prev().prev().find(".autoFinishSwitch").bootstrapSwitch("state")));
    } else {
      $tar.prev().removeClass("hide");
      $tar.prev().prev().addClass("hide");
  ...
  delSmallPic: function (e) {
    var self = this, $tar = $(e.target);
    e.preventDefault();
    var cid = $tar.prev().prev().attr("data-cid");
  ...
  copyProSize: function (e) {
    $(".proSizeName").blur();
    var $tar = $(e.target), cid = $tar.data("cid"), self = this;
    ...
    if (typeof self.sizes.get(cid).attributes.postScript == "undefined") {
      ...
      var currentPostscriptModels = currentPostscript.models, currentPostscriptModelsLen = currentPostscriptModels.length;
      ...
      var newSizeModelPostscript = new sizesAdditionCollection(currentPostscriptModels);
  ...
  removeProSize: function (e) {
    var $tar = $(e.target), cid = $tar.data("cid"), self = this, sizeTr = $("#sizeBox").find("tr").length;
    $.confirm({
      title: false,
      content: "你确定删除吗?",
      buttons: {
          ...
          action: function () {
            if (cid.length > 0) {
              ...
              if (self.sizes.length >= 1) {
                ...
                $("#proStore,#needPay").parents(".form-group").show();
                if ($("#needPay").prop("checked") == true) {
                  $("#generateRandom").parents(".form-group").removeClass("hide");
                }
                sizeBox.addClass("hide");
              }
            }
            if (sizeTr == 1) {
              var addBox = $("#proStyleBtn").parents("div").prev()[0];
              addBox.innerHTML = "选择项&nbsp;" + "<i class=\"glyphicon glyphicon-question-sign\" data-toggle=\"tooltip\" data-placement=\"top\" " + "title=\"可添加信众参与的选项\">" + "</i>：&nbsp;";
              $(addBox).find("[data-toggle=\"tooltip\"]").tooltip();
  ...
  sizesAddAddition: function (e) {
    var self = this;
    e.preventDefault();
    var subCid = $("#sizesPostModal").attr("data-sub-cid"), subModel = self.sizes.get(subCid), subType = subModel.get("subdivide_type");
    self.sizesAddition = new sizesAdditionCollection([new sizesAdditionModel({
      subType: subType
    })]);
    ...
    $("#sizesAdditionTable,#sizesAddAdditionItem").parents(".form-group").css("display", "block").removeClass("hide");
    $("#sizesAddAddition").parents(".form-group").css("display", "none").addClass("hide");
  },
  sizesAddAdditionItem: function (e) {
    ...
    var curSizesAddition = self.sizesAddition, curSizesAdditionModels = curSizesAddition.models, curSizesAdditionModelsLen = curSizesAdditionModels.length;
  ...
  showSizesPostModal: function (e) {
    var $relatedTarget = $(e.relatedTarget), self = this, cid = $relatedTarget.attr("data-cid");
    $(".proSizeName").blur();
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.sizes.get(cid), name = model.get("name");
      $("#sizesPostModal").attr("data-sub-cid", cid);
      $("#sizeName").text("选择项：" + name + " (假如需要用户下单时填写信息，请在此处添加)");
      ...
      if (sizesPostScript) {
        var getSizesAddition = sizesPostScript, getSizesAdditionModels = getSizesAddition.models, getSizesAdditionModelsLen = getSizesAdditionModels.length;
        self.sizesAddition = new sizesAdditionCollection(getSizesAddition.models);
        self.sizesAdditionView = new SizesAdditionsView({
          el: "#sizesAdditionBox",
          model: self.sizesAddition
        });
        ...
        $("#sizesAdditionTable,#sizesAddAdditionItem").parents(".form-group").css("display", "none").addClass("hide");
        $("#sizesAddAddition").parents(".form-group").css("display", "block").removeClass("hide");
      }
    }
  },
  saveSizeInputSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid"), subCid = $("#sizesPostModal").attr("data-sub-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var subModel = self.sizes.get(subCid), subType = subModel.get("subdivide_type"), model = self.sizesAddition.get(cid), inputType = model.get("inputType"), isMust = parseInt($("input[name=inputSizeRequire]:checked").val()), placeholder = $("#inputSizePlaceholder").val(), describtion = $("#inputSizeDescribtion").val();
      if (inputType == 5) {
      ...
      isMust == 0 || isMust == 1 ? model.set("is_must", isMust) : alert("is must has error");
  ...
  saveSizeDateSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.sizesAddition.get(cid), isMust = parseInt($("input[name=dateSizeRequired]:checked").val()), dateRange = parseInt($("input[name=dateSizeRange]:checked").val()), describtion = $("#dateSizeDescribtion").val();
      isMust == 0 || isMust == 1 ? model.set("is_must", isMust) : alert("is must has error");
      dateRange == 0 || dateRange == 1 ? model.set("dataType", ++dateRange) : alert("dateRange has error");
      model.set("describe", describtion);
  ...
  saveSizeSelectSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.sizesAddition.get(cid), isMust = parseInt($("input[name=selectSizeRequire]:checked").val()), dataType = parseInt($("input[name=selectSizeType]:checked").val()), selectInput = $("#selectSizeTextarea").val(), describtion = $("#selectSizeDescribtion").val();
      isMust == 0 || isMust == 1 ? model.set("is_must", isMust) : alert("is must has error");
      dataType == 0 || dataType == 1 ? model.set("dataType", dataType) : alert("dataType has error");
      model.set("describe", describtion);
      if (selectInput == "") {
        var selectInputArr = [];
      } else {
  ...
  saveSizeInstructSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.sizesAddition.get(cid), instruction = $("#sizeInstruction").val(), describtion = $("#sizeInstructionDescribtion").val();
  ...
  saveSizeImagesSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.sizesAddition.get(cid), isMust = parseInt($("input[name=imagesSizeRequire]:checked").val()), maxUploadNum = parseInt($("#inputImagesSizeUploadNum").val()), describtion = $("#imagesSizeDescribtion").val();
      if (maxUploadNum < 1) {
        Toast("最大上传数必须大于0", 2);
        return false;
  ...
  saveSizeRadioSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.sizesAddition.get(cid), isMust = parseInt($("input[name=radioSizeRequired]:checked").val()), describtion = $("#radioSizeDescribtion").val();
      isMust == 0 || isMust == 1 ? model.set("is_must", isMust) : alert("is must has error");
      model.set("describe", describtion);
  ...
  showSizeAdditionModal: function (e) {
    var $relatedTarget = $(e.relatedTarget), $tar = $(e.target), self = this, subCid = $("#sizesPostModal").attr("data-sub-cid"), cid = $relatedTarget.attr("data-cid"), cur_title = $tar.find(".modal-title");
    if (typeof cid == "string" && cid.length > 0) {
      var subModal = self.sizes.get(subCid), subType = subModal.get("subdivide_type"), model = self.sizesAddition.get(cid), inputType = model.get("inputType"), isMust = model.get("is_must"), placeholder = model.get("prompt_text"), describtion = model.get("describe"), dataType = model.get("dataType"), selectInput = model.get("selectInput"), picNum = model.get("pic_num"), font_length = model.get("font_length"), isVerify = model.get("isVerify"), cur_addition_name = config.argument.addition_list[inputType];
      ...
      if (inputType == 5) {
        $("[name=\"sizeTelVerify\"][value=\"" + isVerify + "\"]").prop("checked", true);
      ...
      var $sizesInputRequired = $("#sizesInputRequired"), $sizesInputRequiredFormGroup = $sizesInputRequired.parents(".form-group");
      if (subType == 2 && (inputType == 10 || inputType == 11) || subType == 3 && inputType == 12) {
        $sizesInputRequiredFormGroup.addClass("hide");
      } else {
        $sizesInputRequiredFormGroup.removeClass("hide");
      }
      var $sizesInputMaxNameNum = $("#sizesInputMaxNameNum"), $sizesInputMaxNameNumFormGroup = $sizesInputMaxNameNum.parents(".form-group");
      if (subType == 2 && inputType == 11 || subType == 3 && inputType == 12) {
      ...
      var $sizesInputNameWordLimit = $("#sizesInputNameWordLimit"), $sizesInputNameWordLimitFormGroup = $sizesInputNameWordLimit.parents(".form-group");
      if (subType == 2 && (inputType == 10 || inputType == 11) || subType == 3 && inputType == 12) {
      ...
      var $sizesInputQiFuWishWordLimit = $("#sizesInputQiFuWishWordLimit"), $sizesInputQiFuWishWordLimitFormGroup = $sizesInputQiFuWishWordLimit.parents(".form-group");
      if (subType == 3 && inputType == 15) {
      ...
      var $sizesInputWishWordLimit = $("#sizesInputWishWordLimit"), $sizesInputWishWordLimitFormGroup = $sizesInputWishWordLimit.parents(".form-group");
      if (inputType == 15 && subType != 3) {
  ...
  closeSizePostscript: function (e) {
    ...
    if (self.prepareRemovePostscriptId != "") {
      self.sizePostscript[self.prepareRemovePostscriptId] = self.prepareRemovePostscript;
    }
  },
  sizesAdditionRemove: function (e) {
    var $tar = $(e.target), cid = $tar.data("cid"), allCid = $(".saveAdd").attr("data-cid"), self = this;
    $.confirm({
      title: false,
      content: "你确定删除吗?",
      buttons: {
  ...
  saveAddition: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid"), nameFault = 0;
    if ($("#sizesAddAddition").parents(".form-group").hasClass("hide")) {
      $("#sizesAdditionBox > tr").each(function (index, ele) {
        var curCid = ele.id, curModel = self.sizesAddition.get(curCid), curInputType = curModel.get("inputType"), getName = $(ele).find(".sizeAdditionName").val();
  ...
  onClickSizeTelVerify: function (e) {
    ...
    var self = this, isSubPs = 1, isVerify = parseInt($checkedRadio.val());
    self.initTelPsVerifyMsg(isSubPs, isVerify);
  },
  addAddition: function (e) {
    ...
    $("#additionTable,#addAdditionItem").parents(".form-group").removeClass("hide");
    $("#addAddition").parents(".form-group").addClass("hide");
  },
  ...
  showInputAdditionModal: function (e) {
    var $relatedTarget = $(e.relatedTarget), $tar = $(e.target), self = this, cid = $relatedTarget.attr("data-cid"), cur_title = $tar.find(".modal-title");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.additionItems.get(cid), inputType = model.get("inputType"), isMust = model.get("is_must"), isVerify = model.get("isVerify"), placeholder = model.get("prompt_text"), describetion = model.get("describe"), dataType = model.get("dataType"), selectInput = model.get("selectInput"), picNum = model.get("pic_num"), font_length = model.get("font_length"), cur_addition_name = config.argument.addition_list[inputType];
      cur_title.text(cur_addition_name + "设置");
      ...
      if (inputType == 15) {
        ...
        $("#wishWordLimit").parent().parent("div").removeClass("hide");
      } else {
        $("#wishWordLimit").val("0");
        $("#wishWordLimit").parent().parent("div").addClass("hide");
      }
    } else {
      alert("alert cid is not exsit");
    }
  ...
  removeAddition: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.data("cid");
    $.confirm({
      title: false,
      content: "你确定删除吗?",
      buttons: {
          ...
          action: function () {
            if (cid.length > 0) {
              ...
              if (self.additionItems.length >= 1) {
                ...
                self.additionItems.add(new AdditionModel({}));
                $("#additionTable,#addAdditionItem").parents(".form-group").addClass("hide");
                $("#addAddition").parents(".form-group").removeClass("hide");
              }
            }
          }
        },
  ...
  saveInputSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.additionItems.get(cid), isMust = parseInt($("input[name=inputRequire]:checked").val()), placeholder = $("#inputPlaceholder").val(), describtion = $("#inputDescribtion").val();
  ...
  saveInstructSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.additionItems.get(cid), instruction = $("#instruction").val(), describtion = $("#instructionDescribtion").val();
      model.set("prompt_text", instruction);
      model.set("describe", describtion);
  ...
  saveImagesSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.additionItems.get(cid), isMust = parseInt($("input[name=imagesRequire]:checked").val()), maxUploadNum = parseInt($("#inputImagesUploadNum").val()), describtion = $("#imagesDescribtion").val();
      if (maxUploadNum < 1) {
        Toast("最大上传数必须大于0", 2);
        return false;
      } else if (maxUploadNum > 10) {
  ...
  saveDateSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.additionItems.get(cid), isMust = parseInt($("input[name=dateRequire]:checked").val()), dateRange = parseInt($("input[name=dateRange]:checked").val()), describtion = $("#dateDescribtion").val();
      isMust == 0 || isMust == 1 ? model.set("is_must", isMust) : alert("is must has error");
      dateRange == 0 || dateRange == 1 ? model.set("dataType", ++dateRange) : alert("dateRange has error");
      model.set("describe", describtion);
  ...
  saveRadioSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.additionItems.get(cid), isMust = parseInt($("input[name=radioRequired]:checked").val()), describtion = $("#radioDescribtion").val();
      isMust == 0 || isMust == 1 ? model.set("is_must", isMust) : alert("is must has error");
      model.set("describe", describtion);
  ...
  saveSelectSetting: function (e) {
    var self = this, $tar = $(e.target), cid = $tar.attr("data-cid");
    if (typeof cid == "string" && cid.length > 0) {
      var model = self.additionItems.get(cid), isMust = parseInt($("input[name=selectRequire]:checked").val()), dataType = parseInt($("input[name=selectType]:checked").val()), selectInput = $("#selectTextarea").val(), describtion = $("#selectDescribtion").val();
      isMust == 0 || isMust == 1 ? model.set("is_must", isMust) : alert("is must has error");
      dataType == 0 || dataType == 1 ? model.set("dataType", dataType) : alert("dataType has error");
      model.set("describe", describtion);
      selectInput = selectInput.split("\n");
  ...
  onClickTelVerify: function (e) {
    ...
    var self = this, isSubPs = 0, isVerify = parseInt($checkedRadio.val());
    self.initTelPsVerifyMsg(isSubPs, isVerify);
  },
  ...
  onClickFeedbackType: function (e) {
    ...
    const feedbackImgSrc = Const.feedbackImgArr.find(item => item.value === feedbackType).src;
    $("[data-ele=\"feedback-type\"]").removeClass("active");
    $curTar.addClass("active");
    $feedbackImg.attr("src", feedbackImgSrc);
  },
  if_feedback: function (e) {
    var $tar = $(e.target), cur_id = $tar.attr("id"), ue2 = UE.getEditor("myEditor2", {
  ...
  enter_spcl_sub_ps: function (e) {
    var $tar = $(e.target);
    var curIndex = $tar.attr("data-index"), curImg = $tar.attr("data-src");
    var $container = $("[data-ele=\"spcl-sub-ps-overview-modal-" + curIndex + "\"]");
    if (!!$container.length) {
      $container.stop(true).fadeIn();
      return !1;
    }
    var $container = $(juicer(config.template.component.spclSubPsOverview).render({
      src: curImg,
      index: curIndex
    }));
    $container.appendTo("body").stop(true).fadeIn();
  },
  leave_spcl_sub_ps: function (e) {
    var $tar = $(e.target);
    var curIndex = $tar.attr("data-index");
    $("[data-ele=\"spcl-sub-ps-overview-modal-" + curIndex + "\"]").stop(true).fadeOut();
  },
  onClickSummaryArea: function (e) {
    var $curTar = $(e.currentTarget), value = parseInt($curTar.val(), 10);
    var $summaryAreaTypeContainer = $("[data-ele=\"summary-area-type-container\"]"), $summaryAreaType = $("[data-ele=\"summary-area-type\"]"), $summaryAreaType1 = $("[data-ele=\"summary-area-type\"][data-type=\"1\"]"), $summaryAreaSet = $("[data-ele=\"summary-area-set\"]"), $summaryAreaSet2 = $("[data-ele=\"summary-area-set\"][data-type=\"2\"]"), $targetAmount = $("#target-amount");
    if (value) {
  ...
  onClickSummaryAreaType: function (e) {
    var $curTar = $(e.currentTarget), type = parseInt($curTar.attr("data-type"), 10);
    var $summaryAreaTypeContainer = $("[data-ele=\"summary-area-type-container\"]"), $summaryAreaType = $("[data-ele=\"summary-area-type\"]"), $summaryAreaType2 = $("[data-ele=\"summary-area-type\"][data-type=\"2\"]"), $summaryAreaSet = $("[data-ele=\"summary-area-set\"]"), $summaryAreaSet2 = $("[data-ele=\"summary-area-set\"][data-type=\"2\"]"), $targetAmount = $("#target-amount");
    $summaryAreaType.removeClass("active");
    $curTar.addClass("active");
    if (type === 2) {
      $targetAmount.val("");
  ...
  selectPrinter: function (e) {
    var self = this, $tar = $(e.target), opt = {}, getVal = $tar.val(), get_printer_list = printer_list;
    $("#seletionItemList").find("input").removeAttr("checked");
    $("#printNumber").val(1).selectpicker("refresh");
    $("#print_all").prop("checked", "checked");
    if (getVal >= 0) {
      $("[data-id=\"printNumber\"]").removeAttr("disabled");
      $("[data-type=\"seletionItemList\"]").find("input").removeAttr("disabled");
      opt["printerId"] = getVal;
      doRequest("/zzhadmin/getPrinterStatus/", opt, function (res) {
        $("[data-type=\"exception_tip\"]").eq(0).text(res.msg);
        if (res.msg != "在线，工作状态正常。") {
          $("[data-type=\"seletionItemList\"]").find("input").attr("disabled", "disabled");
          if (res.msg == null) {
      ...
      $.each(get_printer_list, function (index, ele) {
        if (getVal == ele["id"]) {
          var selection_input = $("#seletionItemList").find("input"), curContinuousPrint_num = ele["continuousPrintNum"], isPrintMobile = ele["isPrintMobile"], qrcodePrint = ele["qrcodePrint"];
          var selection_arr = ele["selection"];
          if (typeof curContinuousPrint_num != "undefined" && curContinuousPrint_num.length != 0) {
          ...
          } else if (typeof qrcodePrint != "undefined" && qrcodePrint[0] === 1) {
            $("#print_all").prop("checked", "checked");
          } else if (typeof qrcodePrint != "undefined" && qrcodePrint[0] === 3) {
          ...
          } else if (typeof isPrintMobile != "undefined" && isPrintMobile[0] === 0) {
          ...
          $.each(selection_input, function (subIndex, subEle) {
            ...
            if (typeof selection_arr != "undefined") {
              for (var j = 0; j < selection_arr.length; j++) {
                cur_select_sort == selection_arr[j] && $(subEle).prop("checked", "checked");
              }
            }
          });
        }
  ...
  selectNoSelectionPrinter: function (e) {
    var self = this, $tar = $(e.target), opt = {};
    var $tarParent = $tar.parent(), tarName = $tarParent.find("span").eq(0).text(), ifSelected = $tarParent.find("input").eq(0).prop("checked");
    if (ifSelected) {
      opt["printerId"] = $tarParent.find("input").eq(0).attr("data-id");
      doRequest("/zzhadmin/getPrinterStatus/", opt, function (res) {
        $("[data-type=\"exception_tip\"]").eq(0).text(tarName + ":" + res.msg);
        if (res.msg != "在线，工作状态正常。") {
      ...
      $("[data-type=\"exception_tip\"]").eq(0).text("");
    }
  },
  change_print_number: function (e) {
    var self = this, $tar = $(e.target), getVal = $tar.val(), cur_printer = $("#selectPrinter").val();
    if (getVal == 1 && $("#print_divide").prop("checked") == true) {
      alert("请先把“打印联数”设置为大于1，再设置隔联打印功能。");
      $("#printNumber").val(2).selectpicker("refresh");
      return false;
    }
  ...
  dimension_code_label: function (e) {
    var self = this, $tar = $(e.target), $checkBox = $tar.parents(".form-group").find("#print_divide"), cur_printer = $("#selectPrinter").val();
  ...
  print_tel_label: function (e) {
    var self = this, $tar = $(e.target), cur_printer = $("#selectPrinter").val();
    $.each(printer_list, function (index, ele) {
      if (parseInt(cur_printer) === ele["id"]) {
  ...
  setPrintModal: function (e) {
    var self = this, ifHasSelectItem = !$("#proStyle").parents(".form-group").hasClass("hide");
    var status_str = "<option value=\"1\" data-action=\"print-number\" >1联</option>" + "<option value=\"2\" data-action=\"print-number\" >2联</option>" + "<option value=\"3\" data-action=\"print-number\" >3联</option>" + "<option value=\"4\" data-action=\"print-number\" >4联</option>" + "<option value=\"5\" data-action=\"print-number\" >5联</option>";
    ...
    if ((editType == 1 || editType == 0 || editType == 3) && first_set || has_local_storage && first_set) {
      first_set = false;
      var if_hide_size = $("#proStyle").parents(".form-group").hasClass("hide");
      if (typeof ceremonyMap.printerId == "undefined" && if_hide_size == false) {
        var wrap_arr = [];
        $("#sizeBox > tr").each(function (index, ele) {
          var curCid = $(ele).attr("id"), singleSizesJson = {}, curModel = self.sizes.get(curCid), curPrinterObj = curModel.get("printer");
          $.each(curPrinterObj, function (subIndex, subEle) {
            if (wrap_arr.length == 0) {
              var sub_wrap_obj = {}, sub_wrap_arr_continuousPrint_num = [], sub_wrap_arr_qrcodePrint = [], sub_wrap_arr_isPrintMobile = [], sub_wrap_arr_selection = [];
              sub_wrap_arr_continuousPrint_num.push(subEle["continuousPrintNum"]);
              ...
              sub_wrap_obj["continuousPrintNum"] = sub_wrap_arr_continuousPrint_num;
              ...
              var sub_wrap_arr_continuousPrint_num = [], sub_wrap_arr_qrcodePrint = [], sub_wrap_arr_isPrintMobile = [], sub_wrap_arr_selection = [];
              sub_wrap_arr_continuousPrint_num.push(subEle["continuousPrintNum"]);
              sub_wrap_arr_qrcodePrint.push(subEle["qrcodePrint"]);
              sub_wrap_arr_isPrintMobile.push(subEle["isPrintMobile"]);
              sub_wrap_arr_selection.push(curCid);
              var printer_id_arr = [], if_has_same_id = false, the_same_index = -1;
              ...
              if (if_has_same_id) {
                ...
                wrap_arr[the_same_index]["qrcodePrint"].push(subEle["qrcodePrint"]);
                wrap_arr[the_same_index]["isPrintMobile"].push(subEle["isPrintMobile"]);
                wrap_arr[the_same_index]["continuousPrintNum"].push(subEle["continuousPrintNum"]);
              } else {
                var add_wrap_obj = {};
                add_wrap_obj["id"] = subEle["printerId"];
                add_wrap_obj["continuousPrintNum"] = sub_wrap_arr_continuousPrint_num;
    ...
    if (ifHasSelectItem) {
      ...
      if (printer_list.length > 0 && editType != 1 && editType != 0 && editType != 3 && !has_local_storage) {
        $("#selectPrinter").val("-1");
        ...
        if (printerList.length > 0) {
          ...
          for (var i = 0; i < printerList.length; i++) {
            printerList_str += "<option value=\"" + printerList[i]["id"] + "\" data-action=\"printer-select\" >" + printerList[i]["address"] + "</option>";
            var printer_object = {}, printer_arr = [];
      ...
      var sizesJson = {}, sizesArr = [];
      $("#sizeBox > tr").each(function (index, ele) {
        var curCid = $(ele).attr("id"), singleSizesJson = {};
      ...
      $("[data-type=\"seletionItemList\"]").eq(0).html(template.render(sizesJson));
      $("[data-type=\"seletionItemList\"]").find("input").attr("disabled", "disabled");
      $noSelectPrinterDiv.parent().addClass("hide");
      $("[data-type=\"seletionItemList\"]").eq(0).parent().removeClass("hide");
      $("#selectPrinter").parents(".form-group").removeClass("hide");
      $("#open_print").parents(".form-group").addClass("hide");
      $("[data-type=\"selection_tip\"]").eq(0).removeClass("hide");
      $("[data-type=\"dimension_code\"]").eq(0).removeClass("hide");
      $("[data-type=\"code_tips\"]").eq(0).removeClass("hide");
      self.show_printer_id && $("#selectPrinter").val(self.show_printer_id).selectpicker("refresh").trigger("change");
    } else {
      var print_list_json = {}, print_list_arr = [], template = juicer(config.template.component.print_list);
      $("[data-type=\"seletionItemList\"]").eq(0).html("");
      ...
      $("[data-type=\"seletionItemList\"]").eq(0).parent().addClass("hide");
      $("#selectPrinter").parents(".form-group").addClass("hide");
      $("#open_print").parents(".form-group").removeClass("hide");
      $("[data-type=\"selection_tip\"]").eq(0).addClass("hide");
      $("[data-type=\"noSelectPrinter\"]").find("[data-type=\"no_selection_printer_div\"]").attr("disabled", "disabled");
      var no_selection_list_len = no_selection_list.length;
      if (no_selection_list_len > 0) {
        var if_open_printer = no_selection_list[no_selection_list_len - 1]["isOpenPrinter"], selected_printer_list = no_selection_list[no_selection_list_len - 1]["printerId"], print_number = no_selection_list[no_selection_list_len - 1]["continuousPrintNum"], mobile_print = no_selection_list[no_selection_list_len - 1]["isPrintMobile"], qrcode_print = no_selection_list[no_selection_list_len - 1]["qrcodePrint"];
        if (if_open_printer == 1) {
          $("#open_print").prop("checked", "checked");
          $("[data-type=\"noSelectPrinter\"]").find("[data-type=\"no_selection_printer_div\"]").removeAttr("disabled");
        } else {
          $("#close_print").prop("checked", "checked");
        }
        $("[data-type=\"noSelectPrinter\"]").find("[data-type=\"no_selection_printer_div\"]").removeAttr("checked");
        if (typeof selected_printer_list !== "undefined") {
          for (var j = 0; j < selected_printer_list.length; j++) {
            $("[data-type=\"noSelectPrinter\"]").find("[data-id=\"" + selected_printer_list[j] + "\"]").eq(0).prop("checked", "checked");
          }
        }
        if (typeof print_number !== "undefined") {
          $("#printNumber").val(print_number).selectpicker("refresh");
        }
        if (qrcode_print == 1) {
          $("#print_all").prop("checked", "checked");
  ...
  closeSetPrintModal: function (e) {
    var self = this;
    $("[data-type=\"exception_tip\"]").eq(0).text("");
    var has_selection_list = selection_list, has_not_selection_list = no_selection_list, has_not_selection_list_len = has_not_selection_list.length;
    if (has_not_selection_list_len > 0) {
      overall_if_open_printer = has_not_selection_list[has_not_selection_list_len - 1]["isOpenPrinter"];
  ...
  surePrint: function (e) {
    var self = this, ifHasSelection = !$("[data-type=\"seletionItemList\"]").eq(0).parent().hasClass("hide");
    var transfer = myTool.deepCopy(printer_list);
    origin_printer_list = transfer;
    if (ifHasSelection) {
      var if_has_ercode = $("#print_divide").prop("checked"), print_number = $("#printNumber").val();
      if (if_has_ercode && print_number < 2) {
        alert("请先把“打印联数”设置为大于1，再设置隔联打印功能。");
        return false;
      }
      var get_printer_list = printer_list, selectionList = [], selectionLen = $("[data-type=\"seletionItemList\"]").eq(0).find("input").length;
      $.each(get_printer_list, function (index, ele) {
        $.each(ele["selection"], function (subIndex, subEle) {
          ...
          for (var i = 0; i < selectionLen; i++) {
            if (typeof selectionList[i] == "undefined") {
              ...
              if (selectionList[i]["selection_cid"] == subEle) {
                ...
                selectionList[i]["continuousPrintNum"].push(parseInt(ele["continuousPrintNum"][subIndex]));
                selectionList[i]["qrcodePrint"].push(ele["qrcodePrint"][subIndex]);
                selectionList[i]["isPrintMobile"].push(ele["isPrintMobile"][subIndex]);
          ...
          if (is_new_selection) {
            var add_selection_obj = {}, printer_id_arr = [], continuousPrint_num_arr = [], isPrintMobile_arr = [], qrcodePrint_arr = [];
            printer_id_arr.push(ele["id"]);
            continuousPrint_num_arr.push(parseInt(ele["continuousPrintNum"][subIndex]));
      ...
      $("#sizeBox > tr").each(function (index, ele) {
        var curCid = $(ele).attr("id"), singleSizesJson = {}, curModel = self.sizes.get(curCid);
        ...
        if (sub_wrap_selection.length != 0) {
          for (var i = 0; i < sub_wrap_selection[0]["printerId"].length; i++) {
            ...
            wrap_object["continuousPrintNum"] = sub_wrap_selection[0]["continuousPrintNum"][i];
            wrap_object["qrcodePrint"] = sub_wrap_selection[0]["qrcodePrint"][i];
            wrap_object["isPrintMobile"] = sub_wrap_selection[0]["isPrintMobile"][i];
            wrap_object["printerId"] != "" && (if_set_printer = true) && (self.show_printer_id = wrap_object["printerId"]);
      ...
      var if_open_printer = Number($("#open_print").prop("checked")), qrcode_print, mobile_print, selected_printer_list = [], print_number = "";
      qrcode_print = parseInt($("[name=\"print_method\"]:checked").val());
      mobile_print = parseInt($("[name=\"print_tel_method\"]:checked").val());
      $.each($("[data-type=\"noSelectPrinter\"]").find("[data-type=\"no_selection_printer_div\"]"), function (index, ele) {
        $(ele).prop("checked") && selected_printer_list.push(parseInt($(ele).attr("data-id")));
      });
      ...
      temporary_obj["printerId"] != "" && if_open_printer && (if_set_printer = true) && (self.show_printer_id = temporary_obj["printerId"]);
      self.show_printer_type = 0;
      no_selection_list.push(temporary_obj);
      selection_list = [];
    }
    !if_set_printer && (self.show_printer_id = 0);
    if_set_printer ? self.change_printer_text("已设置") : self.change_printer_text("设置");
    $("#setPrintModal").modal("hide");
  },
  print_switch: function (e) {
    var self = this, if_open = $("#open_print").prop("checked");
    if (if_open) {
      $("[data-type=\"noSelectPrinter\"]").find("[data-type=\"no_selection_printer_div\"]").removeAttr("disabled");
    } else {
      $("[data-type=\"noSelectPrinter\"]").find("[data-type=\"no_selection_printer_div\"]").attr("disabled", "disabled");
  ...
  selection_checkbox: function (e) {
    var self = msgListView, $tar = $(e.target), curPrinterId = $("#selectPrinter").val(), selectionObject = {}, selectionArr = [], selectionPrintNumArr = [], selectionCodeArr = [], selectionMobileArr = [];
    var selectionSort = $tar.attr("data-sort"), selectionPrintNum = $("#printNumber").val(), selectionCode = "", selectionMobile = "";
    ...
    $.each(printer_list, function (index, ele) {
      if (ele["id"] == curPrinterId) {
        if (typeof ele["selection"] != "undefined" && ele["selection"].length != 0) {
          var is_new_selection = false;
          $.each(ele["selection"], function (subIndex, subEle) {
            if (if_checked) {
  ...
  submit: function (e, save_to_template, save_to_draft) {
    var self = this;
    if (ifFirstSave && $("[data-type=\"set_print\"]").eq(0).text() === "设置" && !save_to_template && !save_to_draft) {
      var printerList = "";
      $.ajax({
        type: "get",
        url: "/zzhadmin/getPrinterList/",
    ...
    new StoreImage([window.contentData.detail, window.contentData.payDetail || ""], function (handledContent) {
  ...
  skipSetPrinterAndSave: function () {
    var $modal = $("#notSetPrinterModal"), $submit = $("#submit");
    $modal.modal("hide");
    $submit.click();
  },
  ...
  set_template: function () {
    ...
    $container = $(juicer(config.template.component.templateSetting).render({}));
    $container.appendTo("body").fadeIn();
  },
  ...
  sureCalendar: function (e) {
    var $tar = $(e.target), self = this, cid = $tar.attr("data-cid"), startDate = $("#beginDate").val(), endDate = $("#endDate").val(), startTime = moment(startDate), endTime = moment(endDate), diffTime = endTime.diff(startTime);
    var calendar_check_if_hide = $("#ifAddCalendar").parents(".input-group").hasClass("hide"), calendar_check = $("#ifAddCalendar").prop("checked");
    var set_calendar = 0;
    ...
    if (diffTime < 0) {
      ...
      doRequest("/zzhadmin/addWebSite/", opt, function (msg) {
        Toast("提交成功!");
        self.getUnprintedOrder();
      }, function (msg) {
        alert("提交失败，请联系自在家平台。");
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (unprinted_order_number > 0) {
          $.confirm({
            title: false,
            content: "检测到有" + unprinted_order_number + "个订单未打印，是否打印?",
            animation: "left",
  ...
  read_current_content: function () {
    ...
    if (!(editType == 3 && ifTemplate == 1)) {
      current_content["allow_showVistNum"] = Number($("#show-join-num").bootstrapSwitch("state"));
    ...
    current_content["price"] = $("#suixiInput").parents(".form-group").hasClass("hide") ? 0 : $("#suixiInput").val();
    current_content["stock"] = $("#proStore").val();
    current_content["isAutoFinish"] = $("#suixiInput").next().next().hasClass("hide") ? 0 : Number($("#autoFinishSwitch").bootstrapSwitch("state"));
    current_content["subdivideStr"] = JSON.stringify(self.sizes);
    current_content["postScript"] = JSON.stringify(self.additionItems);
    current_content["showClient"] = $("input[name=participateList]:checked").val();
    current_content["showEndTime"] = $("input[name=remainingTime]:checked").val();
    current_content["opName"] = $("#operation").val();
    current_content["feedback_type"] = $(".selected_feedback_item").attr("data-index") || "";
    ...
    if (is_show_cnt === 1) {
      is_show_cnt = parseInt($("[data-ele=\"summary-area-type\"].active").attr("data-type"), 10);
      if (is_show_cnt === 2) {
        if (isNaN(targetAmount) || targetAmount <= 0 || String(targetAmount).indexOf(".") !== -1) {
          targetAmount = "";
        }
      } else {
        targetAmount = "";
    ...
    if (no_selection_list.length > 0) {
      var no_selection_list_len = no_selection_list.length;
      current_content["printerId"] = no_selection_list[no_selection_list_len - 1]["printerId"];
      current_content["isOpenPrinter"] = no_selection_list[no_selection_list_len - 1]["isOpenPrinter"];
      current_content["continuousPrintNum"] = no_selection_list[no_selection_list_len - 1]["continuousPrintNum"];
      current_content["qrcodePrint"] = no_selection_list[no_selection_list_len - 1]["qrcodePrint"];
      current_content["isPrintMobile"] = no_selection_list[no_selection_list_len - 1]["isPrintMobile"];
    }
      ...
      buttons: {
        ok: {
          ...
          action: function () {
            ...
            $("#show-join-num").bootstrapSwitch("state", !!current_content.allow_showVistNum);
            ...
            add_wrap_obj["continuousPrintNum"] = current_content.continuousPrintNum;
  ...
  startVerify: function () {
    var self = this, detailsEdi = UE.getEditor("myEditor", {
      initialFrameWidth: 700,
      initialFrameHeight: 400
    }), pay_succ_detailsEdi = UE.getEditor("myEditor2", {
      initialFrameWidth: 700,
      initialFrameHeight: 400
    }), details = null, pay_succ_details = "", feedback_type = "", suixiMoney = null, inventory = null, isAutoFinish = null;
    if ($("button[data-id=classification]").length == 0) {
      Toast("请添加分类", 2);
      $("html, body").scrollTop($("#classifiSelect").offset().top);
      return false;
    }
    var foshiName = $("#proName").val(), is_show_participant = $("input[name=participateList]:checked").val(), is_show_time = $("input[name=remainingTime]:checked").val(), typeName = $("button[data-id=classification]")[0].getAttribute("title"), typeIndexFind = 0, typeIndex = 0, subdivideStr = [], typeListString = $("#classification > option").map(function () {
      return $(this).text();
    }).get().join(","), typeIndexListString = $("#classification > option").map(function () {
      return $(this).attr("data-id");
    }).get().join(","), typeList = typeListString.split(","), typeIndexList = typeIndexListString.split(","), inputLength = $("#additionBox > tr").length, inputType = [], inputString = [], additionID = [], isMust = [], inputName = [], prompt_text = [], describtion = [], selectInput = [], font_length = [], isVerify = [], dataType = [], picNum = [], postScript = [], buy_btn_name = $("#operation").val(), start_time = $("#timeStart").val(), end_time = $("#timeLimit").val(), productionImg = [], no_need_pay = false;
    ...
    var startMs = new Date(start_time.replace(/-/g, "/")).getTime(), endMs = new Date(end_time.replace(/-/g, "/")).getTime();
    if (foshiName == "") {
      Toast("请填写标题！", 2);
      $("#proName").parents(".form-group").addClass("has-error");
      $("html, body").scrollTop($("#proName").offset().top);
      return false;
    } else {
      $("#proName").parents(".form-group").removeClass("has-error");
    }
    if ($("#classification").val() === "-1" || $("#classification").val() === null) {
      Toast("请添加佛事分类！", 2);
    ...
    if ($("#sizeBox").parents(".form-group").hasClass("hide")) {
      if ($("#suixiInput").parents(".form-group").hasClass("hide")) {
        suixiMoney = 0;
        isAutoFinish = 0;
        no_need_pay = true;
      } else {
        ...
        if (suixiOrigin == "") {
          ...
          $("#suixiInput").parents(".form-group").addClass("has-error");
          Toast("请输入支付金额！", 2);
          return false;
          ...
          var suixi_match = suixiOrigin.toString().match(myRegExp.suixi_price_right);
          if (suixi_match) {
            suixiOrigin = suixi_match[0];
            suixiOrigin = suixiOrigin.replace(/\，/g, ",");
            var suixiArr = suixiOrigin.split(",");
            if (suixiArr.length > 1) {
              suixiArr[suixiArr.length - 1] == "" && (suixiOrigin = suixiOrigin.substr(0, suixiOrigin.length - 1));
              suixiMoney = "[" + suixiOrigin + "]";
              isAutoFinish = Number($("#autoFinishSwitch").bootstrapSwitch("state"));
            } else {
              suixiMoney = suixiOrigin;
              isAutoFinish = 0;
            }
      ...
      $("#sizeBox > tr").each(function (index, ele) {
        var curCid = ele.id, curModel = self.sizes.get(curCid), subType = curModel.get("subdivide_type"), curPostscript = curModel.get("postScript"), postscriptArr = [];
        if (curPostscript != undefined) {
          var curPostscriptLen = curPostscript.length, curPostscriptArr = $.parseJSON(JSON.stringify(curPostscript));
          for (var i = 0; i < curPostscriptLen; i++) {
            ...
            if (postscriptWrap["inputType"] == 3) {
              ...
              if (selectArr == undefined || selectArr == []) {} else {
                for (var j = 0; j < selectArr.length; j++) {
                  postscriptWrap["prompt_text"][j] = postscriptWrap["prompt_text"][j] || [];
            ...
            if (postscriptWrap["inputType"] == 15 || subType == 3 && postscriptWrap["inputType"] == 12 || subType == 2 && (postscriptWrap["inputType"] == 10 || postscriptWrap["inputType"] == 11)) {
        ...
        var specifiName = curModel.get("name"), specifiPic = curModel.get("pic"), currentPrice = curModel.get("price"), currentPrinter = curModel.get("printer");
        if (typeof currentPrice == "string") {
          var typePrice = currentPrice.indexOf(",") + currentPrice.indexOf("，");
        ...
        if (currentPrice == "") {
          ...
          var select_suixi_match = currentPrice.toString().match(myRegExp.suixi_price_right);
          if (select_suixi_match) {
            ...
            if (typePrice < 0) {
              ...
              currentPrice[currentPrice.length - 1] == "," && (currentPrice = currentPrice.substr(0, currentPrice.length - 1));
        ...
        var specifiStock = curModel.get("stock"), subdivide = {};
        ...
        selection_list.length > 0 && (subdivide["printer"] = curModel.get("printer"));
    ...
    if ($("#additionBox").parents(".form-group").hasClass("hide")) {
      if (no_need_pay) {
        Toast("请添加附言项", 2);
      ...
      $("#additionBox > tr").each(function (index, ele) {
        inputString[index] = $.trim($(ele).eq(0).find("button").eq(0).text());
        if (editType == 1) {
          var addID = $(ele).eq(0).find("img")[0].attributes[1].value;
          additionID[index] = addID;
        }
        var cid = $(ele).attr("id"), modelAll = self.additionItems.get(cid);
        ...
        inputName[index] = $(ele).eq(0).find("input.additionName").eq(0).val();
      ...
      for (var i = 0; i < inputLength; i++) {
        var postScriptInner = {};
        editType == 1 && (additionID[i] != null && additionID[i] != 0) && (postScriptInner["id"] = additionID[i]);
    ...
    if (!$(".picture").val() && !$(".upload_wrap img").length) {
      $("#pictureBox").parents(".form-group").addClass("has-error");
      Toast("请上传封面图片！", 2);
    ...
    if (!$("#proStyle").parents(".form-group").hasClass("hide")) {
      if (!$("#sizesAddAdditionItem").parents(".form-group").hasClass("hide")) {
        ...
        for (var i = 0; i < getSizes.length; i++) {
          ...
          if (typeof getSizePostModel != "undefined") {
            for (var j = 0; j < getSizePostModel.length; j++) {
              var postName = getSizePostModel[j].get("name"), postInputType = getSizePostModel[j].get("inputType");
            ...
            getSizePost.forEach(function (ele) {
              var postName = ele["name"], postInputType = ele["inputType"];
              if (postName == "" && postInputType != 13) {
                $("html, body").scrollTop($("#sizeBox").offset().top);
                Toast("请填写添加项中的附言名称！", 2);
                return false;
    ...
    const ifShowFeedbackImg = parseInt($("[name=\"showFeedbackImg\"]:checked").val(), 10);
    ...
    if (no_selection_list.length > 0) {
      var no_selection_list_len = no_selection_list.length;
      opt["printerId"] = no_selection_list[no_selection_list_len - 1]["printerId"];
      opt["isOpenPrinter"] = no_selection_list[no_selection_list_len - 1]["isOpenPrinter"];
      opt["continuousPrintNum"] = no_selection_list[no_selection_list_len - 1]["continuousPrintNum"];
      opt["qrcodePrint"] = no_selection_list[no_selection_list_len - 1]["qrcodePrint"];
      opt["isPrintMobile"] = no_selection_list[no_selection_list_len - 1]["isPrintMobile"];
    }
    if (!(editType == 3 && ifTemplate == 1)) {
      opt["allow_showVistNum"] = Number($("#show-join-num").bootstrapSwitch("state"));
      opt["custom_introduce"] = $("#summary").val();
    }
    opt.pay_succ_details_flag = parseInt($("input[name=\"if_feedback\"]:checked").attr("value")) || 0;
    window.contentData = opt;
    console.log(window.contentData);
    return true;
  },
  saveOtherSiteAllImagesSuccess: function (save_to_draft) {
    var self = this, params = {}, submit_url = "", start_time = $("#timeStart").val(), end_time = $("#timeLimit").val();
    if (editType == 1) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (save_to_draft) {
          ...
          if (editType != 1 && !(editType == 3 && ifTemplate == 1)) window.sessionStorage.setItem("buddhistVerify", 1);
          if (res.createCalendar !== 1) {
            self.getUnprintedOrder();
          } else {
            if (start_time == "") {
              var startTime = moment(end_time).subtract(2, "day").format("YYYY-MM-DD"), endTime = moment(end_time).format("YYYY-MM-DD");
            } else if (end_time == "") {
              var startTime = moment(start_time).format("YYYY-MM-DD"), endTime = moment(start_time).add(2, "day").format("YYYY-MM-DD");
            } else {
              var endTime = moment(end_time), startTime = moment(start_time), diffTime = endTime.diff(startTime, "days");
              if (diffTime > 6) {
                var endTime = moment(end_time).format("YYYY-MM-DD"), startTime = moment(end_time).subtract(2, "day").format("YYYY-MM-DD");
              } else {
                var endTime = moment(end_time).format("YYYY-MM-DD"), startTime = moment(start_time).format("YYYY-MM-DD");
              }
            }
            $("#ifAddCalendar").parents(".input-group").removeClass("hide");
  ...
  saveOtherSiteAllImagesSuccess_template: function () {
    var self = this, params = {}, submit_url = "";
    window.contentData["startTime"] = "";
    window.contentData["endTime"] = "";
    submit_url = "/zzhadmin/saveCeremonyTemplate/";
    params = {};
  ...
  initialize: function () {
    var self = this, UrlSearch = new self.UrlSearch();
    ...
    var $showJoinNumCtnr = $("#show-join-num-ctnr"), $switch = $("#show-join-num"), $summaryCtnr = $("#summary-ctnr");
    if (editType == 3 && ifTemplate == 1) {
  ...
  render_buddhist_template: function (getContent) {
    ...
    if (typeof getContent.allow_showVistNum !== "undefined") {
      $("#show-join-num").bootstrapSwitch("state", !!getContent.allow_showVistNum);
    }
  ...
  render_ueditor: function () {
    var self = this, ue = UE.getEditor("myEditor", {
      initialFrameWidth: 700,
      initialFrameHeight: 400
    ...
    ue.ready(function () {
      ...
      typeof details == "string" && details.length > 0 ? detailEditor() : detailNullEditor();
      self.hide_loading();
    });
    ue2.ready(function () {
      pay_succ_details = ue2.getContent();
      typeof pay_succ_details == "string" && pay_succ_details.length > 0 ? null : editorSlideTop();
      self.hide_loading();
    });
  },
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data["result"] == 0 || res) {
          ...
          if (foshiId) {
            $("#classification").selectpicker("val", ceremonyMap.ceremonyTypeId);
          }
          ...
          if (res != undefined) {
            ...
            if (typeof imgSrc == "object" && imgSrc.length > 0) {
              ...
              _.each(imgSrc, function (imgsrc) {
                var img = document.createElement("img");
                img.src = imgsrc;
                var top = $(".upload_wrap").find(".upload_box").last().position().top + 5;
                $(img).css({
                  width: "100px",
                  height: "100px",
                  position: "absolute",
                ...
                $(".upload_wrap .upload_box:last-child").addClass("upload_wrap_change ui-state-default").append(img);
  ...
  showimgs: function (getContent) {
    var self = this, imgs = getContent.pics;
    $(".myupload.upload_wrap_change").remove();
    if (imgs == undefined || imgs.length == 0) return false;
    for (var i = 0; i < imgs.length; i++) {
      var $aContent = $(document.createElement("a"));
      $aContent.addClass("myupload upload_box ui-sortable-handle upload_wrap_change ui-state-default posRel");
      ...
      $PicsSpan.attr("style", "position:absolute;right:3px;top:0px;color:rgb(212, 106, 64);");
    ...
    if ($("#pictureBox").find("img").length >= 5) {
      $("#pictureBox")[0].firstElementChild.setAttribute("class", "myupload upload_box ui-sortable-handle hideEle");
    } else {
      $("#pictureBox")[0].firstElementChild.setAttribute("class", "myupload upload_box ui-sortable-handle");
    }
  },
  ...
  showSizes: function (getContent, ifLocalStorage) {
    var self = this, inputSizesOrigin = [];
    if (!ifLocalStorage) {
      inputSizesOrigin = getContent.subdivideStr || [];
    } else {
      inputSizesOrigin = JSON.parse(getContent.subdivideStr) || [];
      if (inputSizesOrigin.length == 0 || (inputSizesOrigin.length == 1 && inputSizesOrigin[0]["cid"] == "" || inputSizesOrigin[0]["name"] == "")) {
        ...
        self.sizesView.render();
        $("#proStyle").parents(".form-group").addClass("hide");
        $("#proStyleBtn").parents("div").prev()[0].innerHTML = "选择项：&nbsp;";
        $("#needPay").parents(".form-group").show();
        $("#proStore").parents(".form-group").show();
        return false;
      }
    }
    ...
    var inputSizesLen = inputSizesOrigin.length, jsonSizes = $.parseJSON(JSON.stringify(inputSizes));
    ...
    for (var i = 0; i < jsonSizes.length; i++) {
      ...
      if (jsonSizes[i].price.toString().indexOf(",") + jsonSizes[i].price.toString().indexOf("，") === -2) {
    ...
    self.sizes.sort();
    self.sizesView.render();
    $("#proStyle").parents(".form-group").hasClass("hide") && $("#proStyleBtn").click();
    if ($(".rePos").length != 0) {
      var right = -$(".rePos").eq(0).position().left;
      $(".delSmallPic").css("right", right);
    }
  ...
  showAddition: function (getContent, ifLocalStorage) {
    var self = this, inputResPos = [];
    if (!ifLocalStorage) {
      inputResPos = getContent.postScript;
    } else {
      inputResPos = JSON.parse(getContent.postScript);
      if (inputResPos.length == 0 || (inputResPos.length == 1 && inputResPos[0]["cid"] == "" || inputResPos[0]["name"] == "")) {
        ...
        self.additionView.render();
        $("#additionTable").parents(".form-group").addClass("hide");
        $("#addAdditionItem").parents(".form-group").addClass("hide");
        $("#addAddition").parents(".form-group").removeClass("hide");
        return false;
      }
    }
    if (inputResPos == undefined || inputResPos.length == 0) return false;
    var inputResPosLen = inputResPos.length, jsonAdditionAdd = $.parseJSON(JSON.stringify(inputResPos));
    for (var j = 0; j < jsonAdditionAdd.length; j++) {
      var inputPrompt = jsonAdditionAdd[j].prompt_text;
      if (Object.prototype.toString.call(inputPrompt) == "[object Array]" && inputPrompt.length) {
        if (typeof inputPrompt[0]["name"] != "undefined") {
          var temp_arr = [];
          for (var k = 0; k < inputPrompt.length; k++) {
            temp_arr.push(inputPrompt[k]["name"]);
    ...
    self.additionView.render();
    $("#additionTable").parents(".form-group").hasClass("hide") && $("#addAddition").click();
  },
  showPrimeSize: function (getContent) {
    var primePrice = getContent.price, primeStock = getContent.stock, primeIsAutoFinish = getContent.isAutoFinish;
    $("#autoFinishSwitch").bootstrapSwitch({
      size: "mini",
      onText: "开",
      offText: "关",
    ...
    if (primePrice != null && primePrice != 0) {
      ...
      if (primePrice.toString().indexOf(",") !== -1 || primePrice.toString().indexOf("，") !== -1) {
        $("#generateRandom").addClass("hide");
        $("#generateRandom").next().removeClass("hide");
      } else {
        $("#generateRandom").removeClass("hide");
        $("#generateRandom").next().addClass("hide");
        $("#autoFinishSwitch").bootstrapSwitch("destroy");
      }
      $("#needPay").prop("checked", "checked");
      $("#suixiInput").parents(".form-group").removeClass("hide");
    } else {
      $("#noNeedPay").attr("checked", "checked");
      $("#suixiInput").parents(".form-group").addClass("hide");
      $("#generateRandom").addClass("hide");
    }
  ...
  showOthers: function (getContent) {
    ...
    var $timeStart = $("#timeStart"), $timeLimit = $("#timeLimit");
    if (editType != 0) {
    ...
    var $showSummary = $("#showSummary"), $hideSummary = $("#hideSummary");
    var $summaryAreaTypeContainer = $("[data-ele=\"summary-area-type-container\"]"), $summaryAreaType = $("[data-ele=\"summary-area-type\"]"), $summaryAreaSet = $("[data-ele=\"summary-area-set\"]"), $summaryAreaSet2 = $("[data-ele=\"summary-area-set\"][data-type=\"2\"]"), $targetAmount = $("#target-amount");
    if (getContent.showStatictics != 0) {
      ...
      $("[data-ele=\"summary-area-type\"][data-type=\"" + getContent.showStatictics + "\"]").addClass("active");
      $summaryAreaTypeContainer.show();
  ...
  setIfOpenFeedback: function (getContent) {
    $("input[name=\"if_feedback\"][value=\"" + getContent.pay_succ_details_flag + "\"]").prop({
      checked: !0
    });
    getContent.pay_succ_details_flag && $("#myEditor2").removeClass("hide");
  },
  setPrinterStatus: function (getContent, ifLocalStorage) {
    var self = this, get_external_printers = getContent["printerId"], if_open_printer = getContent["isOpenPrinter"], get_internal_printer = "";
    if (!get_external_printers) {
      ...
      hasSelectPrinterList[0] && (self.show_printer_id = hasSelectPrinterList[0]);
      self.show_printer_type = 1;
    } else {
      if (if_open_printer) {
        get_external_printers && get_external_printers != "[]" && (self.show_printer_id = get_external_printers);
        self.show_printer_type = 0;
        var add_wrap_obj = {};
  ...
  selection_change_printer_text: function (selectionStatus) {
    var self = this;
    self.show_printer_type == null ? self.change_printer_text("设置") : selectionStatus ? !self.show_printer_type ? self.change_printer_text("设置") : self.change_printer_text("已设置") : !self.show_printer_type && overall_if_open_printer ? self.change_printer_text("已设置") : self.change_printer_text("设置");
  }
});
...
var SizesAdditionView = Backbone.View.extend({
  ...
  render: function () {
    var self = this, template = juicer(config.template.component.sizesAdditionItem_tmp), my_json = self.model.toJSON();
    modelDispose.method.add_default_title_prompt_text(my_json, self.model);
    var html = template.render(my_json), selectpicker = self.$el.html(html).find(".selectpicker");
    self.$el.attr("id", this.model.cid);
    selectpicker.selectpicker({
      noneSelectedText: "未选择"
    });
    my_json.inputType == 3 && my_json.dataType == 2 && (my_json.dataType = 0, self.model.set("dataType", 0));
...
var SizesAdditionsView = Backbone.View.extend({
  initialize: function () {
    this.model.on("change:inputType change:is_must change:dataType change:name change:prompt_text change:selectInput", this.render, this);
  },
  render: function () {
    var self = this;
    ...
    $("#sizesAdditionBox").find("tr").each(function (index, ele) {
      var $ele = $(ele), psId = $ele.attr("id"), subType = self.model.get(psId).get("subType"), inputType = self.model.get(psId).get("inputType");
      if (subType === 2 && (inputType === 10 || inputType === 11) || subType === 3 && (inputType === 12 || inputType === 15) || subType === 4 && (inputType === 4 || inputType === 5 || inputType === 6)) {
...
var SizeView = Backbone.View.extend({
  ...
  className: "ui-state-default",
  render: function () {
    var self = this, template = juicer(config.template.component.size_tmp), my_json = self.model.toJSON();
    var html = template.render(my_json), sizehtml = html.replace(/sizePic0/, "sizePic" + sizeIteration++), selectpicker = self.$el.html(sizehtml).find(".selectpicker");
    this.$el.attr("id", this.model.cid);
    selectpicker.selectpicker({
      noneSelectedText: "未选择"
    });
    $(selectpicker.get(0)).selectpicker("val", self.model.get("subdivide_type"));
    $(selectpicker.get(0)).selectpicker("refresh");
    $(selectpicker.get(0)).on("changed.bs.select", function (e) {
      var subType = parseInt(e.currentTarget.value), pic = self.model.get("pic"), wangshengSrc = "https://pic.zizaihome.com/7b7c6276-0d6f-11e8-8feb-00163e0c001e.png", qifuSrc = "https://pic.zizaihome.com/7d424d78-0d6f-11e8-8feb-00163e0c001e.png";
      var psModelArr = self.model.get("postScript");
      if (!psModelArr) {
        if (subType === 2) {
          psModelArr = new sizesAdditionCollection([new sizesAdditionModel({
            dataType: 2,
            describe: "",
            font_length: 8,
            inputId: "",
        ...
        if (subType === 3) {
          ...
          if (!FMPs.length) {
            FMPs.push(new sizesAdditionModel({
              dataType: 2,
              describe: "",
              font_length: 8,
              inputId: "",
          ...
          if (!XYPs.length) {
            XYPs.push(new sizesAdditionModel({
              dataType: 2,
              describe: "",
              font_length: 20,
              inputId: "",
          ...
          if (!YSRPs.length) {
            YSRPs.push(new sizesAdditionModel({
              dataType: 2,
              describe: "",
              font_length: 8,
              inputId: "",
          ...
          if (!WSZPs.length) {
            WSZPs.push(new sizesAdditionModel({
              dataType: 2,
              describe: "",
              font_length: 8,
              inputId: "",
          ...
          if (!ps4.length) {
            ps4.push(new sizesAdditionModel({
              dataType: 2,
              describe: "",
              font_length: 8,
              inputId: "",
          ...
          if (!ps5.length) {
            ps5.push(new sizesAdditionModel({
              dataType: 2,
              describe: "",
              font_length: 20,
              inputId: "",
          ...
          if (!ps6.length) {
            ps6.push(new sizesAdditionModel({
              dataType: 2,
              describe: "",
              font_length: 20,
              inputId: "",
    ...
    if (this.model.get("pic").length > 0) {
      ...
      this.$el.find(".upload_box").addClass("upload_wrap_change").append(img);
      var sizeSpan = document.createElement("span");
      sizeSpan.setAttribute("class", "glyphicon glyphicon-remove-circle delSmallPic");
      sizeSpan.setAttribute("style", "position:absolute;right:0px;top:0px;color:rgb(212, 106, 64);");
...
var SizesView = Backbone.View.extend({
  initialize: function () {
    this.model.on("change:cid change:name change:stock change:pic change:id change:subdivide_type change:selectInput change:isSingleMoney", this.render, this);
  },
  render: function () {
...
var AdditionView = Backbone.View.extend({
  ...
  className: "ui-state-default",
  render: function () {
    var self = this, template = juicer(config.template.component.additionItem_tmp), my_json = self.model.toJSON();
    modelDispose.method.add_default_title_prompt_text(my_json, self.model);
    var html = template.render(my_json), selectpicker = self.$el.html(html).find(".selectpicker");
    self.$el.attr("id", this.model.cid);
    selectpicker.selectpicker({
    ...
    $(selectpicker.get(0)).on("changed.bs.select", function (e) {
      ...
      if (psType == 2 || psType == 9) {
        $(e.target).parents("td").next().next().find("input").eq(0).attr("placeholder", "");
      }
      self.model.set("inputType", psType);
      if (psType == 15) {
        self.model.set("font_length", 200);
...
var AdditionsView = Backbone.View.extend({
  initialize: function () {
    this.model.on("change:name change:inputType change:is_must change:dataType change:prompt_text change:selectInput", this.render, this);
  },
  render: function () {
    var self = this;
    var fragment = document.createDocumentFragment();
...
$(function () {
  ...
  if (!is_test_environment) {
    ...
    $("#show-join-num").bootstrapSwitch("state", !!ceremonyMap.allow_showVistNum);
    msgListView.showimgs(ceremonyMap);
    $("#summary").val(ceremonyMap.custom_introduce);
    $("#summary").trigger("keyup");
    msgListView.showDetail(ceremonyMap);
    if (typeof ceremonyMap.isOpenPrinter != "undefined" && ceremonyMap.isOpenPrinter != null && editType != 0) {
      var add_wrap_obj = {};
