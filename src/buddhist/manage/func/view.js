...
$.seeView({
  ...
  onClickSearch: function (e) {
    var buddhistTypeId = parseInt($("#buddhist-type-select").val()), buddhistStatus = parseInt($("#buddhist-status-select").val()), searchText = $("#search-input").val();
  ...
  sortByMoney: function (e) {
    func.sortAndRenderList($("#table-title-total-collect"), "orderByCollectMoney");
  },
  onClickOptEdit: function (e) {
    var $curTar = $(e.currentTarget), $curTr = $curTar.parents("tr"), id = parseInt($curTr.attr("data-id")), verify = parseInt($curTar.attr("data-verify"));
    if (verify === 1) {
      window.location.href = "/zzhadmin/createCeremonyIndex/?id=" + id + "&edit=1&verifyId=1";
    } else if (verify === 2) {
      window.location.href = "/zzhadmin/createCeremonyIndex/?id=" + id + "&edit=1&verifyId=2";
    } else {
      window.location.href = "/zzhadmin/createCeremonyIndex/?id=" + id + "&edit=1&verifyId=0";
    }
  },
  onClickOptPromotion: function (e) {
    var $tar = $(e.target), $loadingToastTitle = $("#loading-toast-title"), $loadingToast = $("#loading-toast"), id = parseInt($tar.attr("data-id")), name = Data.handleListData[id].name, status = parseInt($tar.attr("data-status")), title = status === 0 ? "推广" : "预览 (此链接为临时链接，仅供预览)", url = $tar.attr("data-url");
    Promotion.show({
      title: title,
      typeText: "佛事",
      link: url,
      ...
      loadPost: function (callback, title) {
        ...
        $.seeAjax.post("getBuddhistPoster", {
          commodityId: id,
          title: titleStr
        }, function (res) {
          if (res.success) {
  ...
  onClickOptDel: function (e) {
    var $curTar = $(e.currentTarget), $curTr = $curTar.parents("tr"), id = parseInt($curTar.attr("data-id"));
    $.confirm({
      title: false,
      content: "你确定删除吗?",
      buttons: {
  ...
  onClickOptCopy: function (e) {
    var $curTar = $(e.currentTarget), $curTr = $curTar.parents("tr"), id = parseInt($curTr.attr("data-id"));
    $.confirm({
      title: false,
      content: "你确定复制该佛事吗?",
      buttons: {
        ok: {
          text: "确定",
          action: function () {
            window.location.href = "/zzhadmin/createCeremonyIndex/?id=" + id + "&edit=0";
          }
        },
        cancel: {
          text: "取消",
  ...
  onClickOptSetTmpl: function (e) {
    var $curTar = $(e.currentTarget), $curTr = $curTar.parents("tr"), $myModal = $("#set-tmpl-modal"), id = parseInt($curTr.attr("data-id"));
  ...
  onClickOptSchedule: function (e) {
    var $curTar = $(e.currentTarget), $curTr = $curTar.parents("tr"), id = parseInt($curTr.attr("data-id"), 10), $scheduleModal = $("#schedule-modal");
    Data.curBuddhistId = id;
    func.initScheduleModal(0);
    $scheduleModal.modal("show");
  },
  onClickMoreOpt: function (e) {
    var self = this, $tar = $(e.target), getValue = parseInt($tar.val());
    switch (getValue) {
      case 0:
  ...
  onClickConfirmSetTmpl: function (e) {
    var $myModal = $("#set-tmpl-modal"), id = parseInt($myModal.attr("data-id")), name = $("#tmpl-name").val(), params = {};
    params.commodityId = id;
    params.name = name;
    $.seeAjax.post("addTemplate", params, function (res) {
      if (res.success) {
  ...
  onClickCancelSetTmpl: function (e) {
    var $myModal = $("#set-tmpl-modal"), $tmplName = $("#tmpl-name");
    $myModal.attr("data-id", "");
    $tmplName.val("");
    $myModal.fadeOut();
  },
  onShowSetPrtModal: function (e) {
    var $modal = $("#set-prt-modal"), $relatedTar = $(e.relatedTarget), curId = parseInt($relatedTar.attr("data-id"));
    (Data.ifHasSub = $relatedTar.attr("data-ifHasSub") === "true" ? true : false, $modal.attr("data-id", curId));
    $.seeAjax.get("printerList", {}, function (res) {
      if (res.success) {
        ...
        if (res.data.length === 0) {
          ...
          if (Data.ifHasSub) {
            $.seeAjax.post("CommoditySubdivide", {
              commodityId: curId
            }, function (res) {
              Data.getSubListRes = res;
              func.initSetPrtModal(Data.ifHasSub, Data.getPrinterListRes, Data.getSubListRes);
  ...
  onChangeSubPrtSelect: function (e) {
    var $tar = $(e.target), $exceptionTip = $("#exception-tip"), $prtNumSelect = $("#prt-num-select"), $prtTel = $("#prt-tel"), $prtAllQrcode = $("#prt-all-qrcode"), $subCheckbox = $("[data-ele=\"sub-checkbox\"]"), prtId = parseInt($tar.val());
  ...
  onClickSubCheckbox: function (e) {
    var $modal = $("#set-prt-modal"), $target = $(e.target), curId = parseInt($modal.attr("data-id")), prtId = parseInt($("#sub-prt-select").val()), subId = parseInt($target.attr("data-id")), subCont = $target.next().html();
    if ($target.is(":checked")) {
      ...
      Data.handleListData[curId].printerList.map(function (value) {
        if (value.printerId === prtId) {
          ...
          value.notBoundSubNameList.splice(value.notBoundSubNameList.indexOf(subCont), 1);
        }
      });
    } else {
      Data.localPrtCfg[prtId].subList.splice(Data.localPrtCfg[prtId].subList.indexOf(subId), 1);
      Data.handleListData[curId].printerList.map(function (value) {
        if (value.printerId === prtId) {
  ...
  onChangePrtNumSelect: function (e) {
    var $curTar = $(e.currentTarget), continuousPrintNum = parseInt($curTar.val()), qrcodePrint, data;
    if (Data.ifHasSub) {
  ...
  onClickPrtQrcode: function (e) {
    var $curTar = $(e.currentTarget), qrcodePrint = parseInt($curTar.val()), continuousPrintNum, data;
    if (Data.ifHasSub) {
      var $subPrtSelect = $("#sub-prt-select"), prtId = parseInt($subPrtSelect.val());
      data = Data.localPrtCfg[prtId];
    } else {
      data = Data.localPrtCfg;
    }
    ...
    if (continuousPrintNum === 1 && qrcodePrint === 2) {
      alert("请先把“打印联数”设置为大于1，再设置隔联打印功能。");
      $("[name=\"prt-qrcode\"][value=\"" + data.qrcodePrint + "\"]").prop("checked", "checked");
    } else {
      data.qrcodePrint = qrcodePrint;
    }
  },
  onClickPrtTel: function (e) {
    var $curTar = $(e.currentTarget), isPrintMobile = parseInt($curTar.val()), data;
    if (Data.ifHasSub) {
      var $subPrtSelect = $("#sub-prt-select"), prtId = parseInt($subPrtSelect.val());
      data = Data.localPrtCfg[prtId];
    } else {
  ...
  onClickNoSubPrtSwitch: function (e) {
    var $curTar = $(e.currentTarget), isOpenPrinter = parseInt($curTar.val()), data = Data.localPrtCfg, $modal = $("#set-prt-modal"), curId = parseInt($modal.attr("data-id")), $noSubPrtCheckBox = $("[data-ele=\"no-sub-prt-checkbox\"]");
    if (isOpenPrinter) {
      $noSubPrtCheckBox.removeAttr("disabled");
      var printerListArr = [];
      $noSubPrtCheckBox.each(function (index, ele) {
        if ($(ele).is(":checked")) {
          var printerId = parseInt($(ele).attr("data-id")), printerName = $(ele).next().html();
          printerListArr.push({
            printerId: printerId,
            notBoundSubNameList: [],
            subNameList: [],
  ...
  onClickNoSubPrtCheckbox: function (e) {
    var $curTar = $(e.currentTarget), prtId = parseInt($curTar.attr("data-id")), prtName = $curTar.next().html(), ifChecked = $curTar.prop("checked"), $modal = $("#set-prt-modal"), $exceptionTip = $("#exception-tip"), curId = parseInt($modal.attr("data-id")), data = Data.localPrtCfg;
    if (ifChecked) {
      $.seeAjax.post("printerStatus", {
        printerId: prtId
      }, function (res) {
        if (res.success) {
  ...
  onClickSavePrtSet: function (e) {
    var $modal = $("#set-prt-modal"), curId = parseInt($modal.attr("data-id"));
    if (Data.ifHasSub) {
      ...
      if (Data.localPrtCfg.isOpenPrinter && !Data.localPrtCfg.printerId.length) {
        Toast("请至少选择一台打印机", 2);
        return;
      }
    }
    var params = func.createUpdatePrtParams(Data.ifHasSub, curId, Data.localPrtCfg, Data.getSubListRes.data);
    $.seeAjax.post("addAndUpdateCommodity2Printer", params, function (res) {
      if (res.success) {
        var printerDataResult = func.handleGetListPrinterData(Data.handleListData[curId]);
  ...
  onClickModalHeadNav: function (e) {
    var $curTar = $(e.currentTarget), index = parseInt($curTar.attr("data-index"), 10);
  ...
  onClickAddImg: function (e) {
    ...
    var upload = new ChooseImage({
      ...
      onSubmit: function (resData) {
        ...
        $curTar.parent().prev().append(htmlStr);
      }
    });
    upload.show();
  },
  onClickDelImg: function (e) {
    var $curTar = $(e.currentTarget), $curImgCell = $curTar.parents("[data-ele=\"img-cell\"]");
    $curImgCell.remove();
  },
  onClickDelUploadVideo: function (e) {
    const $curTar = $(e.currentTarget);
    const $btn = $curTar.parent().parent().next().find("[data-ele=\"add-video\"]");
    Data.curUploadJqXHR.abort();
    $("[data-ele=\"video-upload-loading\"]").remove();
    $btn.show();
  },
  onClickDelVideo: function (e) {
    var $curTar = $(e.currentTarget), $curVideoCell = $curTar.parents("[data-ele=\"video-cell\"]");
    $curVideoCell.remove();
  },
  onClickPlayVideo: function (e) {
    var $curTar = $(e.currentTarget), $curVideoCell = $curTar.parents("[data-ele=\"video-cell\"]"), src = $curVideoCell.attr("data-src");
  ...
  onClickSaveSchedule: function (e) {
    ...
    var $modalBody = $("#create-schedule-modal-body"), params = {};
  ...
  onClickEditScheduleItem: function (e) {
    var $curTar = $(e.currentTarget), $curScheduleItem = $curTar.parents("[data-ele=\"schedule-item\"]"), $curScheduleContent = $curScheduleItem.find("[data-ele=\"schedule-content\"]"), scheduleContent = $.trim($curScheduleContent.text()), htmlStr = tpl.scheduleContentTextarea.render({
  ...
  onClickSaveScheduleItem: function (e) {
    var $curTar = $(e.currentTarget), $curScheduleItem = $curTar.parents("[data-ele=\"schedule-item\"]"), $scheduleContent = $curScheduleItem.find("[data-schedule-content]"), scheduleContent = $scheduleContent.val(), scheduleId = parseInt($curScheduleItem.attr("data-id"), 10), params = {};
    params.buddhistId = Data.curBuddhistId;
    ...
    var $ifPush = $curScheduleItem.find("[name=\"if-push-" + scheduleId + "\"]:checked");
    if ($ifPush.length) {
  ...
  onClickCancelScheduleItem: function (e) {
    var $curTar = $(e.currentTarget), $curScheduleItem = $curTar.parents("[data-ele=\"schedule-item\"]"), scheduleId = parseInt($curScheduleItem.attr("data-id"), 10), data = Data.buddhistScheduleListHandleData[scheduleId], htmlStr = tpl.scheduleItem.render(data);
    $curScheduleItem.replaceWith(htmlStr);
  }
});
