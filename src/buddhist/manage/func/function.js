...
func.initBuddhistVerifyModal = function () {
  ...
  if (parseInt(buddhistVerify)) {
    toastr.success("工作人员已收到通知，请耐心等待处理 <br> 周一至周五：10:00-19:30 <br> 周末节假日：10:00-17:00 （整点审核）", "提交成功，审核中", {
      timeOut: 20000,
      closeButton: !0,
      escapeHtml: !1
    });
...
func.renderVideoPlayer = function (src) {
  ...
  if (!Data.videoPlayer) {
    ...
    const videoPlayer = window.videojs($videoPlayer.get(0), {
      controls: !0,
      preload: "auto",
      autoplay: !1,
      fluid: !1,
...
func.initVideoUpload = function ($btn) {
  zzhUpload($btn, function (url) {
    console.log("上传成功");
    $("[data-ele=\"video-upload-loading\"]").remove();
    $btn.parent().prev().append(tpl.scheduleVideoCell.render({
      src: url
    }));
    $btn.show();
  }, function (e, data) {
    ...
    componentOption: {
      add: function (e, data) {
        ...
        if (uploadError.length) {
          ...
          $btn.parent().prev().append(tpl.scheduleVideoUploadLoading.render({}));
          $btn.hide();
        }
      }
    }
...
func.renderBuddhistTypeSelect = function (res) {
  var $select = $("#buddhist-type-select"), htmlStr = "<option value=\"-1\">全部</option>";
...
func.renderBuddhistStatus = function () {
  var $select = $("#buddhist-status-select"), htmlStr = "<option value=\"0\">全部</option>";
...
func.handleGetListPrinterData = function (item) {
  var prtText = "", ifHasAddPrt = false, ifHasSub = false, ifHasSubNameList = false, ifHasNotBoundSubNameList = false, prtLen = item.printerList.length, result = {};
  item.printerList.sort(function (a, b) {
    return a.printerId - b.printerId;
  });
  if (prtLen > 0) {
    item.printerList.map(function (value) {
      if (value.subNameList.length === 0 && value.notBoundSubNameList.length === 0) {
        prtText += "<dl>";
        ...
        value.subNameList.map(function (valueA) {
          prtText += "<dd class=\"clearfix\"><i class=\"icon-container green-select-icon\"></i>" + valueA + "</dd>";
        });
        value.notBoundSubNameList.map(function (valueB) {
          ifHasNotBoundSubNameList = true;
          prtText += "<dd class=\"clearfix\"><i class=\"icon-container yellow-selected-icon\"></i>" + valueB + "</dd>";
        });
        prtText += "</dl>";
      } else {
        ifHasNotBoundSubNameList = true;
...
func.handleGetListStatusData = function (item) {
  var remainTimeText = "", endTimeArr = [], remainTimeArr = [], result = {};
  if (!item.isEnd) {
    item.endTime.toString().split("-").map(function (value, index, arr) {
      endTimeArr.push(parseInt(value));
    });
    if (endTimeArr[0] === 2099) {
      remainTimeText = "长期佛事活动。";
    } else {
      remainTimeArr = [endTimeArr[0] - commonVars.today.year, endTimeArr[1] - commonVars.today.month, endTimeArr[2] - commonVars.today.day];
      if (remainTimeArr[0] > 0) {
        remainTimeText = "距离结束还剩" + remainTimeArr[0] + "年" + remainTimeArr[1] + "月" + remainTimeArr[2] + "天";
      } else if (remainTimeArr[1] > 0) {
        remainTimeText = "距离结束还剩" + remainTimeArr[1] + "月" + remainTimeArr[2] + "天";
      } else {
...
func.renderList = function (res) {
  var $ctnr = $("#table-body"), htmlString = "", ifHasPrt = res.ifHasPrinter;
  res.data.map(function (item) {
    var printerDataResult = func.handleGetListPrinterData(item), statusDataResult = func.handleGetListStatusData(item);
...
func.refreshList = function () {
  var $ctnr = $("#table-body"), $pagination = $("#pagination-container");
  $pagination.html("");
...
func.endItem = function (params, callback) {
  $.seeAjax.post("endItem", params, function (res) {
    if (res.success) {
      callback && callback(res);
    } else {
      res.message && commonFunc.alert(res.message);
...
func.initSetPrtModal = function (ifHasSub, prtListData, subListData) {
  var $noSubPrtSwitchFormGroup = $("#no-sub-prt-switch-form-group"), $subPrtFormGroup = $("#sub-prt-form-group"), $noSubPrtFormGroup = $("#no-sub-prt-form-group"), $subTip = $("#sub-tip"), $subListFormGroup = $("#sub-list-form-group"), $exceptionTip = $("#exception-tip"), $prtNumSelect = $("#prt-num-select"), $prtTel = $("#prt-tel"), $prtAllQrcode = $("#prt-all-qrcode");
  ...
  if (ifHasSub) {
    ...
    var $subPrtSelect = $("#sub-prt-select"), $subListCtnr = $("#sub-list-ctnr");
    ...
    $noSubPrtCtnr.find("[data-ele=\"no-sub-prt-checkbox\"]").attr("disabled", "disabled");
  }
};
...
func.renderSetPrtModal = function (ifHasSub, cfg, prtId) {
  ...
  if (ifHasSub) {
    ...
    if (typeof curPrtId !== "undefined") {
      var $subPrtSelect = $("#sub-prt-select"), $subCheckbox = $("[data-ele=\"sub-checkbox\"]"), $exceptionTip = $("#exception-tip");
      ...
      cfg[curPrtId].subList.map(function (subId) {
        $("[data-ele=\"sub-checkbox\"][data-id=\"" + subId + "\"]").prop("checked", "checked");
      });
    } else {
      return;
    }
    ...
    cfg.printerId.map(function (value) {
      $("[data-ele=\"no-sub-prt-checkbox\"][data-id=\"" + value + "\"]").prop("checked", "checked");
    });
  }
  $("#prt-num-select").val(continuousPrintNum).selectpicker("refresh");
  $("[name=\"prt-qrcode\"][value=" + qrcodePrint + "]").prop("checked", "checked");
  $("[name=\"prt-tel\"][value=\"" + isPrintMobile + "\"]").prop("checked", "checked");
};
func.getAndRenderPrtCfg = function (buddhistId) {
  $.seeAjax.post("CommodityPrinter", {
    commodityId: buddhistId
  }, function (res) {
    if (res.success) {
      Data.localPrtCfg = func.createLocalPrtCfg(Data.ifHasSub, Data.getPrinterListRes.data, res.data);
...
func.createUpdatePrtParams = function (ifHasSub, buddhistId, localPrtCfg, subListData) {
  var params = {};
...
func.renderTr = function (id) {
  var $curTr = $("tr[data-id=\"" + id + "\"]"), htmlStr = "";
...
func.initScheduleModal = function (index) {
  var $modalHeadNav = $("[data-ele=\"modal-head-nav\"]"), $scheduleModalBody = $("[data-ele=\"schedule-modal-body\"]");
  $modalHeadNav.removeClass("active");
  $("[data-ele=\"modal-head-nav\"][data-index=\"" + index + "\"]").addClass("active");
...
func.initCreateScheduleModalBody = function () {
  ...
  var $scheduleImgCell = $modalBody.find("[data-ele=\"schedule-img\"]").parents("[data-ele=\"img-cell\"]");
  $scheduleImgCell.remove();
  var $scheduleVideoCell = $modalBody.find("[data-ele=\"schedule-video\"]").parents("[data-ele=\"video-cell\"]");
...
func.initScheduleListModalBody = function () {
  var $modalBody = $("#schedule-list-modal-body"), htmlStr = "", params = {
