...
export default class extends Component {
  ...
  itemJsx = item => {
    ...
    const curTemplateTypeObj = systemTemplateTypeData.find(
      item2 => item2.id === item.templateTypeId
    );
    ...
    return (
      ...
      >
        ...
        <div className={styles.content}>
          ...
          {curTemplateTypeObj ? (
            <div className={styles.btn}>{curTemplateTypeObj.name}</div>
          ) : (
            ''
          )}
        </div>
  ...
  contentJsx() {
    ...
    return (
      <div className={styles.container}>
        {data.map(item => this.itemJsx(item))}
      </div>
    );
  }
