      ...
      <div>
        ...
        >
          <el-option
            ...
            :disabled="!item.status || (item.type === 1 && type === 'set')"
            :key="item.id"
            :label="
...
export default {
  ...
  computed: {
    ...
    type() {
      return this.$store.state.printerDialogType;
    },
  },
  created() {
    this.getPrinterList();
  ...
  methods: {
    getPrinterConfig() {
      seeAjax('getPrinterConfig', {}, res => {
        if (res.success) {
          ...
          res.data.map(item => {
            printer.push(item.printerId);
          });

          this.printer = id;
          this.printNum = printNum;
          this.printQrcode = printQrcode;
          this.printTel = printTel;
        } else {
          Notification({
            title: '提示',
            message: '您还没有添加打印机',
    ...
    onClickSave() {
      ...
      // 组装数据
      let printerSnList = [];
      let selectPrinter = this.printerList.filter(item => item.id == printer);
      selectPrinter.forEach(item => {
        printerSnList.push({
          id: item,
