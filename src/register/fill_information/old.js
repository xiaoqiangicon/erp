...
var foshiId = null, editTag = null, error = 0, uploadOthersId = 6;
$(function () {
  if (window.location.pathname == "/zzhadmin/templeApplySubmitData/" && window.navigator.userAgent.indexOf("Chrome/45") == -1) {
...
$("#templeInfor").on("blur", ".required", function (e) {
  var $tar = $(e.target), $tarValue = $tar.val();
  if ($tarValue == "") {
    $tar.addClass("redBorder").nextAll(".nullError").removeClass("hide");
    error = 1;
  } else {
    $tar.removeClass("redBorder").nextAll(".nullError").addClass("hide");
    error = 0;
  }
});
$("#templeInfor").on("blur", "#templeName", function (e) {
  ...
  if (!(/^[\u4e00-\u9fa5]+$/gi).test($tarValue)) {
    $tar.addClass("redBorder").nextAll(".nullError_msg").removeClass("hide");
    error = 1;
  } else {
    $tar.removeClass("redBorder").nextAll(".nullError_msg").addClass("hide");
    error = 0;
  }
});
...
$(".pwdRow ").on("blur", "#affirmNewPwd", function (e) {
  var $tar = $(e.target), $tarValue = $tar.val(), $newPwd = $("#newPwd").val();
  if ($tarValue != $newPwd) {
    $tar.addClass("redBorder").nextAll(".nullError_msg").removeClass("hide");
    error = 1;
  } else {
...
var View = Backbone.View.extend({
  ...
  initialize: function (e) {
    ...
    if (window.location.pathname == "/zzhadmin/" || window.location.pathname == "/") {
    ...
    if (window.location.pathname == ("/zzhadmin/templeApplySubmitData/" || "/zzhadmin/templeRegisterResult/") || foshiId != undefined) {
      $.ajax({
        type: "get",
        ...
        success: function (data) {
          ...
          $("#login").addClass("hide");
          $("#logined").removeClass("hide").text(name);
        },
        error: function (msg) {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (window.location.pathname == "/zzhadmin/templeApplyFillInfo/") {
            var name = data.data.name, province = data.data.province, city = data.data.city, area = data.data.area, address = data.data.address, registrant = data.data.registrant, registrantMobile = data.data.registrantMobile, placeNo = data.data.placeNo, mobile = data.data.mobile, email = data.data.email, corporation = data.data.corporation;
            $("#login").addClass("hide");
            $("#logined").removeClass("hide").text(name);
            ...
            if (registrant == "") {
              $("#templeTel").attr("disabled", "disabled").addClass("frozen");
            }
            ...
            $("#templePhone").val(registrantMobile).attr("disabled", "disabled").addClass("frozen");
            $("#templeVertifica").attr("disabled", "disabled").addClass("frozen");
            $("#templePassword").attr("disabled", "disabled").addClass("frozen");
            $("#templePassword2").attr("disabled", "disabled").addClass("frozen");
            $("#vertifica").css("background-color", "#aaa").attr("disabled", "disabled");
            $("#isLegal").parents(".form-group").addClass("hide");
          } else {
            var placeNoPic = data.data.placeNoPic, bankPic = data.data.bankPic, idCardPic = data.data.idCardPic, certificatePic = data.data.certificatePic, otherCertificatePic = data.data.otherCertificatePic;
            ...
            $("[data-type=\"otherUploadData\"]").find(".uploadImg").remove();
            otherCertificatePic = otherCertificatePic.split(",");
            var otherPic_obj = {};
            otherPic_obj["data"] = otherCertificatePic;
            var template_boxs = juicer(config.template.component.uploadBoxs), html_boxs = template_boxs.render(otherPic_obj);
  ...
  showLegal: function (e) {
    ...
    $("#isLegal").find("input").prop("checked", "checked");
    $("#noLegal").find("input").prop("checked", "");
    $("#templeSignin").parents(".form-group").addClass("hide");
    $("#templePhone").parents(".form-group").addClass("hide");
    $(".loginTip").text("法人手机号码为登录账号");
  },
  hideLegal: function (e) {
    var self = this, $tar = $(e.target);
    $("#noLegal").find("input").prop("checked", "checked");
    $("#isLegal").find("input").prop("checked", "");
    $("#templeSignin").parents(".form-group").removeClass("hide");
    $("#templePhone").parents(".form-group").removeClass("hide");
      ...
      success: function (e) {
        ...
        $.ajaxFileUpload({
          ...
          success: function (data, status) {
            var jsonValue = data.split(",")[1].split("\""), picsValue = jsonValue[3], $img = $("#" + sizePicId).parents(".uploadButton").next().find("img");
    ...
    $tar.checkFileTypeAndSize({
      ...
      success: function (e) {
        ...
        $.ajaxFileUpload({
          ...
          data: {},
          success: function (data, status) {
            var jsonValue = data.split(",")[1].split("\""), picsValue = jsonValue[3], $img_length = $("[data-id=\"uploadOthers\"]").length, $img = $("[data-id=\"uploadOthers\"]").eq($img_length - 1);
            $img.attr("src", picsValue);
            var delete_template = juicer(config.template.component.deleteUpload), delete_html = delete_template.render({});
            $img.parent().append(delete_html);
            var template = juicer(config.template.component.uploadBox), html = template.render({});
  ...
  sendVertifica: function (e) {
    var self = this, $tar = $(e.target), vertificaError = 0, mobile = $("#templeTel").val(), isLegal = $("[name=\"Legal\"]:checked").val(), registrantMobile = $("#templePhone").val();
    var opt = {};
    if (isLegal == 1) {
    ...
    if (vertificaError == 0) {
      doRequest("/zzhadmin/templeApplySendCode/", opt, function (msg) {
        $("#vertifica").attr("disabled", "disabled").attr("style", "background-color:#bbb");
        self.timer(60);
        Toast(msg.msg);
      }, function (msg) {
        Toast("网络出错,发送失败", 0);
  ...
  timer: function (e) {
    var self = this, time = e;
    if (time == 0) {
      $("#vertifica").removeAttr("disabled").text("重新发送").attr("style", "");
      $("#getVertifiCode").removeAttr("disabled").text("重新发送").attr("style", "");
    } else {
      $("#vertifica").text(time);
  ...
  submitFillNext: function (e) {
    var self = this, $tar = $(e.target);
    var name = $("#templeName").val(), placeNo = $("#templeCode").val(), province = $("[data-id=\"province\"]").val(), city = $("[data-id=\"city\"]").val(), area = $("[data-id=\"district\"]").val(), address = $("#address").val(), corporation = $("#templeLegal").val(), mobile = $("#templeTel").val(), email = $("#templeEmail").val(), isLegal = $("[name=\"Legal\"]:checked").val(), registrant = $("#templeSignin").val(), registrantMobile = $("#templePhone").val(), templeVertifica = $("#templeVertifica").val(), pwd = $("#templePassword").val(), pwd2 = $("#templePassword2").val();
    error = 0;
    ...
    var noSubProvince = ["北京市", "上海市", "天津市", "重庆市"], noSubCity = ["济源市", "潜江市", "神农架林区", "天门市", "仙桃市", "东莞市", "中山市", "东沙群岛", "白沙黎族自治县", "保亭黎族苗族自治县", "昌江黎族自治县", "澄迈县", "儋州市", "定安县", "东方市", "乐东黎族自治县", "临高县", "陵水黎族自治县", "琼海市", "琼中黎族苗族自治县", "屯昌县", "万宁市", "文昌市", "五指山市", "嘉峪关市", "阿拉尔市", "北屯市", "可克达拉市", "昆玉市", "石河子市", "双河市", "铁门关市", "图木舒克市", "五家渠市"];
    var isNoSubSelect = function (province, city) {
      if (noSubProvince.indexOf(province) !== -1 || noSubCity.indexOf(city) !== -1) {
        return true;
      } else {
        return false;
      }
    ...
    if (error == 0) {
      ...
      doRequest(URL, opt, function (msg) {
        Toast("上传成功!");
        if (editTag == 1) {
          var curId = foshiId;
          window.location.href = "/zzhadmin/templeApplySubmitData/?edit=1&id=" + curId;
        } else {
          var curId = msg.data.id;
          window.location.href = "/zzhadmin/templeApplySubmitData/?id=" + curId;
        }
      }, function (msg) {
        if (msg.result == -8) {
          Toast("用户已存在", 2);
  ...
  submitPrev: function (e) {
    var self = this, $tar = $(e.target), curId = foshiId;
    window.location.href = "/zzhadmin/templeApplyFillInfo/?id=" + curId + "&edit=1";
  },
  submitNext: function (e) {
    var self = this, $tar = $(e.target), picsError = 0;
    var placeNoPic = $("[data-id=\"upload1\"]").attr("src"), bankPic = $("[data-id=\"upload2\"]").attr("src"), certificatePic = $("[data-id=\"upload3\"]").attr("src"), idCardPic = $("[data-id=\"upload4\"]").attr("src"), otherCertificatePic = "";
    $.each($("[data-id=\"uploadOthers\"]"), function (index, ele) {
      otherCertificatePic += $(ele).attr("src") + ",";
    });
    otherCertificatePic = otherCertificatePic.substr(0, otherCertificatePic.length - 2);
    if (placeNoPic == "") {
    ...
    if (picsError == 0) {
      ...
      doRequest("/zzhadmin/templeVerifyPic/", opt, function (msg) {
        Toast("上传成功!");
        var curId = foshiId;
  ...
  suppleInfo: function (e) {
    var self = this;
    window.location.href = "/zzhadmin/templeApplySubmitData/?edit=1&id=" + verifyData.id;
  },
  ...
  confirmVertifi: function (e) {
    var self = this, $tar = $(e.target), phoneNumber = $("#phoneNumber").val(), inputCode = $("#input").val().toUpperCase(), opt = {};
    if (inputCode.length <= 0) {
      ...
      doRequest("/zzhadmin/templeApplySendCode/", opt, function (msg) {
        $("#getVertifiCode").attr("disabled", "disabled").attr("style", "background-color:#bbb");
        self.timer(60);
        Toast(msg.msg, 2);
      }, function (msg) {
        Toast("发送失败", 0);
  ...
  resetPwd: function (e) {
    var self = this, $tar = $(e.target), resetError = 0;
    var phoneNumber = $("#phoneNumber").val(), shortMessage = $("#shortMessage").val(), newPwd = $("#newPwd").val(), affirmNewPwd = $("#affirmNewPwd").val();
    ...
    if (resetError == 0) {
      ...
      opt["pwd"] = newPwd;
      doRequest("/zzhadmin/editPwd/", opt, function (msg) {
        Toast("上传成功!");
  ...
  getVertifiCode: function (e) {
    var self = this, phoneNumber = $("#phoneNumber").val();
    if (phoneNumber == "" || $("#phoneNumber").hasClass("redBorder")) {
      $("body").scrollTop($("#phoneNumber").offset().top);
      Toast("请填写注册人手机号码！", 2);
      return false;
  ...
  showPassword: function (e) {
    var $tar = $(e.target), $passwordInput = $("#templePassword"), $passwordType = $("#templePassword").attr("type");
  ...
  showPassword2: function (e) {
    var $tar = $(e.target), $passwordInput = $("#templePassword2"), $passwordType = $("#templePassword2").attr("type");
    if ($passwordType == "text") {
      $passwordInput.attr("type", "password");
      $tar.attr("class", "showPassword");
    } else {
