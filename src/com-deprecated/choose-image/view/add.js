...
seeView({
  events: {
    ...
    'click [data-zzh-choose-image-upload-image-delete]':
      'onClickUploadImageDelete',
  ...
  extractImageSuccessful: function(url, id, $addPopup) {
    let self = this;
    seeAjax(
      'zzhChooseImageExtract',
      {
        file: url,
      },
      res => {
        if (res.success)
          self.saveExtractedImageSuccessful(res.url, id, $addPopup);
      }
    );
  },
  saveExtractedImageSuccessful: function(url, id, $addPopup) {
    $addPopup.find('[data-zzh-choose-image-extract-content]').append(
      uploadImageCellTpl({
        image: url,
      })
    );
  ...
  onClickAddSubmit: function(e) {
    ...
    let $addPopup = $this.parents('[data-zzh-choose-image-add-popup]');
    let id = parseInt($addPopup.attr('data-zzh-choose-image-add-popup'));
    let $imageCells = $addPopup.find(
      '[data-zzh-choose-image-upload-image-cell]'
    );
    let images = [];
  ...
  saveImageSuccessful: function(id, images) {
    ...
    if ($tab1_1.hasClass('active')) requestList(id);
    else $tab1_1.trigger('click');
    if (images.length) {
      let result = [];
      images.map(image => {
        result.push({
