...
var View = Backbone.View.extend({
  ...
  countPeople: function () {
    var trueContent = $("#textCtn").val(), contact_person = "", contact_tel = "";
    var constitutionName = $.trim($(".countFirstLetter").text());
    constitutionName = "【" + constitutionName + "】";
    var msgContent = constitutionName + trueContent, singleMsgNumber = parseInt(msgContent.length / 70) + 1, sendMethod = "";
    ...
    if (trueContent) {
      if (sendMethod === "phone") {
        var phoneInput = $("#phoneInput").val(), phoneArr = phoneInput.split("\n"), phoneArrLen = phoneArr.length;
        if (phoneArrLen > 0) {
          ...
          if (verifyPhone) {
            ...
            if (curMsgNumber < msgNumber) {
              ...
              $("#numHint .peopleCnt").text(phoneArrLen + "个人，消耗" + msgNumber + "条短信");
              $("#countPeople").modal();
            }
          } else {
        ...
        var selectedGroup = $("#groupId").val(), selectedGroupLen = selectedGroup.length;
        if (selectedGroupLen) {
          var curMsgNumber = parseInt($(".remainingText").text()), msgNumber = singleMsgNumber * selectedGroupLen;
          if (curMsgNumber < msgNumber) {
            $("#insufficient").modal();
          } else {
  ...
  confSubmit: function () {
    var trueContent = $("#textCtn").val(), contact_person = "", contact_tel = "";
    var constitutionName = "【" + $.trim($(".countFirstLetter").text()) + "】", msgContent = constitutionName + trueContent, sendMethod = "";
    if ($("#phoneSend").attr("checked") === "checked") {
      sendMethod = "phone";
    } else {
      sendMethod = "group";
  ...
  initialize: function () {
    ...
    var arrLen = [];
    arrLen[0] = $(".m-pagination-page").attr("style") == "display: none;" ? 0 : $(".m-pagination-page").outerWidth(true);
    arrLen[1] = $(".m-pagination-size").attr("style") == "display: none;" ? 0 : $(".m-pagination-size").outerWidth(true);
    arrLen[2] = $(".m-pagination-jump").attr("style") == "display: none;" ? 0 : $(".m-pagination-jump").outerWidth(true);
    arrLen[3] = $(".m-pagination-info").attr("style") == "display: none;" ? 0 : $(".m-pagination-info").outerWidth(true);
    var len = arrLen.length, length = 0;
    for (var i = 0; i < len; i++) {
