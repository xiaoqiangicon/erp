...
var if_first_init = true, activityId;
var View = Backbone.View.extend({
  ...
  beforeBatchDelVol: function () {
    var self = this, ids = self.selectedVolunteerIds();
    if (ids.length > 0) {
  ...
  labelSingle: function (e) {
    ...
    var $currentTar = $(e.currentTarget), $tar = $(e.target), labelTags = $currentTar.find("[data-type=\"tags\"]"), labelIds = [];
    for (i = 0; i < labelTags.length; i++) {
      labelIds.push(labelTags.eq(i).attr("data-id"));
    }
    var inputLabel = $(".js_tag_putOn_tags input"), labelArrLen = labelIds.length;
  ...
  sureDelVol: function (e) {
    e.stopPropagation();
    var self = this, $tar = $(e.target), delItemId = [];
  ...
  search: function () {
    var self = this, searchItem = $("#search").val(), opt = {};
    opt["pageNumber"] = 0;
    opt["pageSize"] = self.getNumber;
    opt["sex"] = $("#sex").prev().find(".selected").attr("data-original-index");
    ...
    if (searchItem == "") {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data["result"] >= 0) {
            var labelName = $("#label").selectpicker("val"), label = $(".label-group button.selectpicker").attr("title");
            $(".volGroupTitle").attr("data-id", labelName).text(label);
            $("[data-type=\"volunteerAmount\"]").text("共" + data.total + "人");
            self.displayOperationPanle();
            var template = juicer(config.template.component.volunteer_panel_temp), opt = {};
            opt["data"] = data.data;
            var HTMLtemplate = template.render(opt);
            $(".volGroupWarp").html(HTMLtemplate);
            self.delVolPopStart();
            var firstSize = self.getNumber + 0.1, pageSize = 3, count = Math.floor(firstSize / pageSize), pageNum = data.pageNumber, dataLen = data.data.length;
            self.scroll2getData(count, pageNum, dataLen);
          }
          if (pageNum < 0 && (dataLen = 0)) {
            $(".volGroupWarp").html("<div class=\"nothingFound\"> <span class=\"icon bell_icon\"></span><span class=\"nothingTips\">抱歉，没有找到相关信息</span> </div>");
          }
        },
        error: function () {
          Toast("网络服务失败，搜索失败", 0);
  ...
  UserInfoRedirect: function (e) {
    var $tar = $(e.currentTarget), id = $tar.attr("data-id"), target = e.target, labelTar = $(target).parents(".labelSingle");
    if (target.className != "mask" && target.className != "maskDown" && target.className != "del-icon" && target.className != "labelSingle" && !labelTar.length) {
      window.open("/zzhadmin/volunteer_loggingData/?activityId=" + activityId + "&volunteerId=" + id);
    }
  },
  tagAllCondition: function () {
    var labelName = $(".volGroupTitle").attr("data-id"), defaultHtml = $(".volGroupTitle").html();
    return labelName == "全部义工" || defaultHtml == "全部义工";
  },
  displayOperationPanle: function () {
    var self = this, status = ["none", "block"];
  ...
  delVolunteer: function () {
    var self = this, ids = self.selectedVolunteerIds(), opt = {};
    opt["volunteerId"] = ids;
    opt = JSON.stringify(opt);
    if (ids) {
      $.ajax({
        ...
        success: function (msg) {
          ...
          $(".action").removeClass("backgroundGreen").removeClass("white");
        },
        error: function (msg) {
  ...
  delLabel: function () {
    var self = this, labelId = $(".volGroupTitle").attr("data-id");
    $(".cancelDel").unbind("click").click(function () {
      $(".del").popover("hide");
    });
    $(".sureDelBtn").unbind("click").click(function () {
      if (self.tagAllCondition()) {
  ...
  exportExcel: function () {
    ...
    let end = $("#endDate").val();
    let sexValue = $("#sex").prev().find(".selected").attr("data-original-index");
    let sex;
    if (sexValue == "1") {
      sex = "男";
    } else if (sexValue == "2") {
    ...
    let url = `/zzhadmin/downloadVolunteerList?from_age=${fromAge}&to_age=${toAge}` + `&start=${start}&end=${end}&searchContent=${searchContent}&activityId=${activityId}` + `&tagId=${tagId}${sex ? `&sex=${sex}` : ""}`;
    console.log(url);
    window.open(url);
  },
  ...
  addLabel: function () {
    var self = this, labelSize = $("#label").find("option").length, selectedItemLength = $(".mask.selected").length;
  ...
  elClick: function (e) {
    var $tar1 = $(e.target).parents(".labelOperation"), $tar2 = $(e.target).parents(".popover "), $tar3 = $(e.target).parents("#batchOperation "), $tar4 = $(e.target).parents(".weChat-msg-del-box ");
        ...
        if (msg["result"] >= 0) {
          ...
          if (groups) {
            ...
            for (var i = 0; i < groups.length; i++) {
              group_str += "<option value=\"" + groups[i]["id"] + "\">" + groups[i]["name"] + "</option>";
            }
  ...
  first_init: function () {
    ...
    $("#groupingModal").on("show.bs.modal", function (e) {
      $("#batchOperation").popover("hide");
      var labelGroup = $("#label option"), groupLabel = {};
      labelGroup.each(function (index, DomEle) {
        var id = $(DomEle).val(), name = $(DomEle).text();
        if (id == "0") {} else {
      ...
      var template = juicer(config.template.component.labelGroupTemp), opt = {};
      opt["list"] = groupLabel;
      var HTMLtemplate = template.render(opt);
  ...
  requireGroupList: function () {
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < groups.length; i++) {
          var dom2 = $(" <li><span>" + groups[i].name + " </span><span class='label label-success backgroundGreen groupCnt'>" + groups[i].cnt + "</span></li>");
  ...
  editTitleContent: function (img_id, img_name) {
    $("#InputEdit").attr("value", img_name).attr("data-id", img_id);
  },
  rename: function (e) {
    var me = this, labelName = $(".volGroupTitle").attr("data-id"), label = $(".volGroupTitle").text();
    me.editTitleContent(labelName, label);
    $(".cancelRename").unbind("click").click(function () {
      $(".rename").popover("hide");
    });
    $(".sureRenameBtn").unbind("click").click(function () {
      ...
      if (newLabelName == "" || newLabelName.length < 2) {
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data.result >= 0) {
              me.loadSelectData();
              $(".volGroupTitle").attr("data-id", labelName).text(newLabelName);
              $(".rename").popover("hide");
  ...
  sureAddLabel: function () {
    var labelName = $("#labelName").val(), self = this;
    if (labelName.length < 2) {
      ...
      $.ajax({
        ...
        success: function (msg) {
          Toast(msg["msg"]);
          if (msg["result"] >= 0) {
            var id = msg["data"]["id"], str = "<option value=\"" + id + "\">" + labelName + "</option>";
            $("#label").append(str);
            $("#label").selectpicker("refresh");
            $("#newModal").modal("hide");
          }
  ...
  selected: function (e) {
    ...
    $tar.parents(".jsMaskWrp").find(".mask").toggleClass("selected");
  },
  ...
  queryVolenteer: function (signup_way) {
    ...
    if (signup_way == 3) {
      ...
      var conHeight = Math.abs($(window).height() - $(".leftBox").height() - $(".leftBox").offset().top);
    }
    var perWidth = 300, perHeight = 110, colNumber = Math.floor(conWidth / perWidth), rowNumber = Math.floor(conHeight / perHeight);
  ...
  checkScrollSide: function () {
    var $aPin = $(".volGroupWarp>.volGroupPanel"), lastPinH = $aPin.last().offset().top + Math.floor($aPin.last().height() / 2), documentH = $(window).height();
    return lastPinH < documentH ? true : false;
  },
  popoverStart: function () {
    $("#batchOperation").popover({
      html: true,
      trigger: "manual",
  ...
  initialize: function (e) {
    ...
    $("#recordData").prop("href", "/zzhadmin/volunteer_loggingData/?activityId=" + activityId);
    ...
    $("#from_age").bind("click blur", function () {
      var val = parseInt($(this).val()), bigVal = parseInt($("#to_age").val());
      if (val < 0) {
        Toast("年龄不能小于0", 2);
        $(this).val(0);
    ...
    $("#to_age").bind("click blur", function () {
      ...
      if (smallVal >= 0 && smallVal <= val) {} else if (val == 1) {
        $("#from_age").val(0);
      } else if (isNaN(val)) {
        $("#to_age").val(smallVal);
      } else {
  ...
  sureAddGrouping: function (e) {
    var self = this, ids = self.selectedVolunteerIds(), userId = $(e.currentTarget).attr("data-id"), selectedIDs = null, url = "", checkedGroups = $(".js_tag_putOn_tags input[type='checkbox']:checked"), slectedID = [], checkID = [];
  ...
  delVolPopStart: function () {
    $(".del-icon").popover({
      ...
      $(this).popover("show");
      $(".del-icon").not(this).popover("hide");
      $(".rename").popover("hide");
      $("#batchOperation").popover("hide");
    });
  },
  ...
  scroll2getData: function (count, pageNum, dataLen) {
    var self = this, scrollStatus = true;
    function scrollData() {
      var winH = $(window).height(), pageH = $(document.body).height(), scrollT = document.documentElement.scrollTop || document.body.scrollTop, aa = (pageH - winH - scrollT) / winH;
      if (aa < 0.25) {
        if (document.body.clientWidth < 1400) {
          var conWidth = 930;
        } else if (document.body.clientWidth < 1682) {
        ...
        var perWidth = 300, colNumber = Math.floor(conWidth / perWidth);
        if (pageNum > 0 || dataLen > 0) {
          var opt = {};
  ...
  query: function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data["result"] >= 0) {
          var labelName = $("#label").prev().find(".selected").find(".text")[0].innerText, label = $("#label").val();
          $(".volGroupTitle").attr("data-id", label).text(labelName);
          $("[data-type=\"volunteerAmount\"]").text("共" + data.total + "人");
          self.displayOperationPanle();
          var template = juicer(config.template.component.volunteer_panel_temp), opt = {};
          opt["data"] = data.data;
