...
var upload = function (el, option, done, progress) {
  ...
  var $el, elType = typeof el;
  if (elType == "string") $el = $(el); else if (elType == "object" && el.nodeType == 1 && typeof el.nodeName == "string") $el = $(el); else if (el instanceof $) $el = el; else throw new Error("common/upload_file: 未知父容器；父容器必须是：selector选择器, dom对象, jquery对象");
