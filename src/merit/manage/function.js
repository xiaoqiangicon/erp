...
func.init = function () {
  ...
  $("#filter-select").val(1).selectpicker("refresh");
...
func.renderAllTbody = function (res) {
  var $ctnr = $("#all-ctnr"), $selectNum = $ctnr.find("[data-ele=\"select-num\"]"), $table = $("#all-table"), $tbody = $("#all-tbody"), $pagination = $ctnr.find("[data-ele=\"pagination-ctnr\"]"), $selectAll = $table.find("[data-ele=\"select-all\"]"), htmlStr = "";
  $selectNum.html(0);
  ...
  if (res.total > Data.getListParams.pageSize) {
    func.createPagination($pagination, res.total, Data.getListParams.pageSize, Data.getListParams.page, function (page) {
...
func.renderTagList = function (handleData, tagId) {
  var $ctnr = $("#list-ctnr"), $curTagName = $("[data-ele=\"cur-tag-name\"]"), $activeTag, activeTagId, htmlStr = "";
...
func.renderTagTbody = function (res) {
  var $ctnr = $("#tag-ctnr"), $selectNum = $ctnr.find("[data-ele=\"select-num\"]"), $table = $("#tag-table"), $tbody = $("#tag-tbody"), $pagination = $ctnr.find("[data-ele=\"pagination-ctnr\"]"), $selectAll = $table.find("[data-ele=\"select-all\"]"), htmlStr = "";
  $selectNum.html(0);
  $selectAll.prop("checked", false);
  res.data.map(function (item) {
...
func.createPagination = function ($pagination, totalCount, pageSize, currentPage, callback) {
  var totalPages = Math.ceil(totalCount / pageSize), pagination = new Pagination($pagination, {
