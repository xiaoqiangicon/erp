...
$.seeView({
  events: {
    ...
    "click [data-ele=\"show-add-to-follow-modal\"]": "onClickShowAddToFollowModal",
  ...
  onClickAllTab: function (e) {
    var $tabs = $(".tab"), $allTab = $("#all-tab"), $allCtnr = $("#all-ctnr"), $tagCtnr = $("#tag-ctnr");
  ...
  onClickTagTab: function (e) {
    var $tabs = $(".tab"), $tagTab = $("#tag-tab"), $allCtnr = $("#all-ctnr"), $tagCtnr = $("#tag-ctnr"), $tagTableCtnr = $("#tag-table-ctnr");
    $tabs.removeClass("active");
  ...
  onClickSearchSubmit: function (e) {
    e.preventDefault();
    var $filterSelect = $("#filter-select"), selectVal = parseInt($filterSelect.val()), $filterInput = $("#filter-input"), inputVal = $.trim($filterInput.val()), $allCtnr = $("#all-ctnr"), $tagCtnr = $("#tag-ctnr");
    Data.getListParams.page = 0;
    if (selectVal === 1) {
  ...
  onClickSearchReset: function (e) {
    e.preventDefault();
    var $filterInput = $("#filter-input"), $searchSubmit = $("#search-submit"), $allCtnr = $("#all-ctnr"), $tagCtnr = $("#tag-ctnr");
    $filterInput.val("");
    Data.getListParams.page = 0;
    delete Data.getListParams.searchText;
    delete Data.getListParams.number;
  ...
  onClickShowAddToFollowModal: function (e) {
    var curTabId = $("[data-ele=\"tab\"].active").attr("id"), $tagTab = $("#tag-tab"), $curTable = curTabId === "all-tab" ? $("#all-table") : $("#tag-table"), $selectCheckbox = $curTable.find("[data-ele=\"select-merit-checkbox\"]:checked"), $modal = $("#add-to-follow-modal");
  ...
  onClickSaveAddToFollow: function (e) {
    var curTabId = $("[data-ele=\"tab\"].active").attr("id"), $curTable = curTabId === "all-tab" ? $("#all-table") : $("#tag-table"), $selectCheckbox = $curTable.find("[data-ele=\"select-merit-checkbox\"]:checked"), $modal = $("#add-to-follow-modal"), $select = $modal.find("[data-ele=\"tag-select\"]"), $curTag = $("[data-ele=\"tag-list\"].active"), curTagId = parseInt($curTag.attr("data-id")), tagIds = $select.val(), userIds = [];
    if (tagIds) {
      tagIds = tagIds.join(",");
  ...
  sortByMoney: function (e) {
    var $curTar = $(e.currentTarget), $curTable = $curTar.parents("table"), $titleMoney = $curTable.find("[data-ele=\"table-title-collect-money\"]"), $titleAmount = $curTable.find("[data-ele=\"table-title-total-amount\"]"), $allCtnr = $("#all-ctnr"), $tagCtnr = $("#tag-ctnr");
  ...
  onClickSelectAll: function (e) {
    var $curTar = $(e.currentTarget), $curTable = $curTar.parents("table"), $selectNum = $("[data-ele=\"select-num\"]"), ifChecked = $curTar.prop("checked"), $selectMeritCheckbox = $curTable.find("[data-ele=\"select-merit-checkbox\"]");
    $selectMeritCheckbox.prop("checked", ifChecked);
    var checkedNum = $curTable.find("[data-ele=\"select-merit-checkbox\"]:checked").length;
    $selectNum.html(checkedNum);
  },
  onClickSelectMeritCheckox: function (e) {
    var $curTar = $(e.currentTarget), $curTable = $curTar.parents("table"), $selectNum = $("[data-ele=\"select-num\"]"), checkedNum = $curTable.find("[data-ele=\"select-merit-checkbox\"]:checked").length, ifExistSelect = !!checkedNum;
    $selectNum.html(checkedNum);
  },
  onClickTagList: function (e) {
    var $curTar = $(e.currentTarget), $tagLists = $("[data-ele=\"tag-list\"]"), $curTagName = $("[data-ele=\"cur-tag-name\"]"), $tagTableCtnr = $("#tag-table-ctnr"), $selectNum = $tagTableCtnr.find("[data-ele=\"select-num\"]"), tagId = parseInt($curTar.attr("data-id"));
  ...
  onClickAddTag: function (e) {
    var $addTagCtnr = $("#add-tag-ctnr"), $addTagInput = $("#add-tag-input");
    $addTagCtnr.show();
    $addTagInput.focus();
  },
  onKeyupAddTagInput: function (e) {
    var $saveAddTag = $("#save-add-tag"), keyCode = e.keyCode;
    if (keyCode === 13) {
  ...
  onMousedownSaveAddTag: function (e) {
    Data.lockBlur = !!1;
    var $addTagCtnr = $("#add-tag-ctnr"), $addTagInput = $("#add-tag-input"), $listCtnr = $("#list-ctnr"), name = $.trim($addTagInput.val());
    if (name.length) {
      api.updateTag({
        tagId: 0,
        tagName: name
  ...
  onClickRenameTag: function (e) {
    ...
    if ($curTag.length) {
      var tagId = parseInt($curTag.attr("data-id")), handleData = Data.getTagHandleData;
  ...
  onKeyupEditTagInput: function (e) {
    var $saveEditTag = $("#save-edit-tag"), keyCode = e.keyCode;
    if (keyCode === 13) {
  ...
  onBlurEditTagInput: function (e) {
    ...
    if (!Data.lockBlur) {
      var $editTagCtnr = $("#edit-tag-ctnr"), $editTagInput = $("#edit-tag-input"), tagId = parseInt($editTagInput.attr("data-id")), hanleData = Data.getTagHandleData;
  ...
  onMousedownSaveEditTag: function (e) {
    Data.lockBlur = !!1;
    var $editTagCtnr = $("#edit-tag-ctnr"), $editTagInput = $("#edit-tag-input"), tagId = parseInt($editTagInput.attr("data-id")), name = $.trim($editTagInput.val()), handleData = Data.getTagHandleData, $curTagName = $("[data-ele=\"cur-tag-name\"]");
    if (name.length) {
      api.updateTag({
        tagId: tagId,
        tagName: name
  ...
  onClickDelTag: function (e) {
    var $curTag = $("[data-ele=\"tag-list\"].active"), $tagTableCtnr = $("#tag-table-ctnr"), tagId = parseInt($curTag.attr("data-id")), handleData = Data.getTagHandleData;
  ...
  onClickRemoveFromTag: function (e) {
    var $tagTable = $("#tag-table"), $curTag = $("[data-ele=\"tag-list\"].active"), $selectCheckbox = $tagTable.find("[data-ele=\"select-merit-checkbox\"]:checked"), tagId = parseInt($curTag.attr("data-id")), userIds = [];
    if ($selectCheckbox.length) {
