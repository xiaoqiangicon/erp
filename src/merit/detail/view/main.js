...
$.seeView({
  ...
  onClickSaveAddToFollow: function (e) {
    var $modal = $("#add-to-follow-modal"), $select = $("[data-ele=\"tag-select\"]"), tagIds = $select.val(), userIds = Data.getUserInfoParams.id.toString();
    if (tagIds) {
      tagIds = tagIds.join(",");
      api.addMeritToGroup({
        tagIds: tagIds,
  ...
  onClickOptDetl: function (e) {
    e.preventDefault();
    var $curTar = $(e.currentTarget), typeId = parseInt($curTar.attr("data-typeId")), orderId = parseInt($curTar.attr("data-orderId"));
    if (typeId === 2) {
