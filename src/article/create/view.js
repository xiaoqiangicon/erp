...
$.seeView({
  ...
  onClickEditCategory: function (e) {
    var $popup = $("[data-popup=\"category\"]"), $popupBody;
    if (!$popup.length) {
  ...
  onClickPopup: function (e) {
    var $this = $(e.target), $popup = $(e.currentTarget), close = !!$this.attr("data-popup-close") || !!$this.attr("data-popup-overlay");
    if (close) {
      $popup.hide();
      this.revertWindowScroll();
      commonFunc.removeCloseWindowHint();
      $popup.attr("data-type") == "article-view" && (location.href = data.indexUrl);
    }
  },
  onClickPopupCategoryAdd: function (e) {
    var $this = $(e.target), $input = $("[data-popup-category-add-input]"), handling = !!parseInt($this.attr("data-handling")), value = $input.val().trim();
    if (handling || !value) return;
    $this.text("添加中...");
    $this.attr({
      "data-handling": 1
    });
    $.seeAjax.get("addCategory", {
      name: value
    }, function (res) {
      var item = {}, $popupBody = $("[data-popup-category-body]"), $inputCategory = $("#input-category");
      $this.text("添加");
      $this.attr({
        "data-handling": 0
      });
  ...
  onClickPopupCategoryCellModify: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-popup-category-cell-modify")), $input = $("[data-popup-category-cell-input=\"" + id + "\"]"), handling = !!parseInt($this.attr("data-handling")), value = $input.val().trim();
    if (handling || !value) return;
    $this.text("修改中...");
    $this.attr({
      "data-handling": 1
    });
    $.seeAjax.get("modifyCategory", {
      id: id,
      name: value
    }, function (res) {
      var currentItem;
  ...
  onClickPopupCategoryCellDelete: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-popup-category-cell-delete")), handling = !!parseInt($this.attr("data-handling"));
    if (handling) return;
  ...
  afterUploadCover: function (result) {
    var $coverImage = $("#upload-cover-image"), $coverShow = $("#upload-cover-show"), $uploadContainer = $("#upload-cover-container");
    if (!result.length) {
      return;
    }
  ...
  onClickSave: function (e) {
    var self = this, $this = $(e.target), saveType = parseInt($this.attr("data-save")), handling = !!parseInt($this.attr("data-handling"));
    if (handling) return;
    ...
    new StoreImage(window.contentData.content, function (handledContent) {
  ...
  saveOtherSiteAllImagesSuccess: function () {
    var self = this, urlName;
    window.contentData.status = data.saveType;
    parseInt(window.contentData.status) == 2 && !window.contentData.publishTime && (window.contentData.publishTime = commonVars.today.display + " 00:00:00");
    data.formatedParams.action == "update" ? (window.contentData.id = data.formatedParams.articleId, urlName = "updateArticle") : urlName = "addArticle";
    $.seeAjax.post(urlName, window.contentData, function (res) {
      $("[data-save=\"" + data.saveType + "\"]").text(data.saveTypeOriginText[data.saveType - 1]);
      $("[data-save]").attr({
        "data-handling": 0
      });
      if (res.success) {
  ...
  onSaveDataSuccess: function (articleId, status) {
    var url = data.shareLink + articleId, $viewPopup;
    $viewPopup = $(tpl.viewPopup.render({
      id: articleId,
      link: url,
      status: status
    }));
  ...
  onKeepData: function () {
    window.contentData = {};
    var title = $("#input-title").val().trim(), category = parseInt($("#input-category").val()), cover = $("#upload-cover-image").attr("src"), content = data.editor.getContent(), introduction = $("#input-introduction").val(), usePay = parseInt($("input[name=\"use-pay\"]:checked").val()), payDisplay = $("#input-pay-display").val(), payDefault = $("#input-pay-default").val(), payGuide = $("#input-pay-guide").val(), showCoverInDetail = parseInt($("input[name=\"show-cover-in-detail\"]:checked").val()), showTempleWebsite = parseInt($("input[name=\"show-temple-website\"]:checked").val()), publishTime = $("#input-publish-time").val(), showTitle = parseInt($("input[name=\"show-title\"]:checked").val()), showSupport = parseInt($("input[name=\"show-support\"]:checked").val());
    if (!title) {
    ...
    window.contentData = {
      title: title,
      category: category,
      cover: cover.indexOf("?") > -1 ? cover.slice(0, cover.indexOf("?")) : cover,
      content: content,
  ...
  onClickInputPayDefaultRandom: function (e) {
    var $this = $(e.target), $input = $("#input-pay-default");
    $input.val(data.randomMoney);
  },
  onKeyupInputPayDefault: function (e) {
    var $this = $(e.target), value = $this.val(), newValue;
    if (data.randomInputInvalidRegExp.test(value)) {
      newValue = value.replace(data.randomInputInvalidRegExp, "");
      data.chineseCommaRegExp.test(newValue) && (newValue = newValue.replace(data.chineseCommaRegExp, ","));
      $this.val(newValue);
      return;
    }
    data.chineseCommaRegExp.test(value) && (value = value.replace(data.chineseCommaRegExp, ","), $this.val(value));
  },
  onChangeUsePay: function (e) {
    var $this = $("[name=\"use-pay\"]:checked"), value = parseInt($this.val()), $paySections = $("[data-pay-select-section]");
    !!value ? $paySections.show() : $paySections.hide();
  },
  onClickSwitchQrcode: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-switch-qrcode")), type = parseInt($this.attr("data-type")), url = data.shareLink + id, $qrcodeContainer = $("[data-qrcode-container=\"" + id + "\"]"), activeSwitch = $("[data-switch-qrcode=\"" + id + "\"].active");
