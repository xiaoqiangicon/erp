...
$.seeView({
  ...
  onClickPopup: function (e) {
    var $this = $(e.target), close = !!$this.attr("data-popup-close") || !!$this.attr("data-popup-overlay");
    if (close) {
      $(e.currentTarget).hide();
    }
  },
  onClickArticleView: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-article-view")), status = parseInt($this.attr("data-status")), url = data.shareLink + id, $viewPopup = $("[data-popup=\"" + id + "\"][data-type=\"article-view\"]");
    if (!$viewPopup.length) {
  ...
  onClickSelectCategory: function (e) {
    var $this = $(e.target), category = parseInt($this.attr("data-select-category")), $category = $("#category"), currentCategory = parseInt($category.attr("data-category"));
  ...
  onClickSelectStatus: function (e) {
    var $this = $(e.target), status = parseInt($this.attr("data-select-status")), $status = $("#status"), currentStatus = parseInt($status.attr("data-status"));
  ...
  onClickArticleEdit: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-article-edit")), status = parseInt($this.attr("data-status"));
    location.href = data.updateLink + id + "&status=" + status;
  },
  onClickArticleCopy: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-article-copy"));
    location.href = data.copyLink + id;
  },
  onClickArticleDelete: function (e) {
    var self = this, $this = $(e.target), id = parseInt($this.attr("data-article-delete"));
    commonFunc.confirm("确定删除这篇文章吗？", function () {
      $.seeAjax.get("deleteArticle", {
        id: id
      }, function () {
