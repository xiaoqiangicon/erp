...
util.requestCellData = function (row, column, online, callback) {
  if (online) {
    $.seeAjax.get("onlineCellInfo", {
      id: data.currentRegionId,
      row: row,
      column: column
    }, function (res) {
      if (res.success) {
        data.cellData[data.currentRegionId][func.getRowColumnKey(row, column)] = res.data;
        callback && callback();
      } else {
...
util.fillCreatePopup = function (onlyFillSequence) {
  ...
  if (data.currentRow && data.currentColumn) {
    cellData = data.cellData[data.currentRegionId][func.getRowColumnKey(data.currentRow, data.currentColumn)];
  }
  ...
  $("#create-modal-time").val(cellData && cellData.endTime || "").prop({
    disabled: !!data.currentEditIsOnline
  });
  var $forever = $("#create-modal-forever");
  if (data.currentEditIsOnline) $forever.addClass("hide"); else $forever.removeClass("hide");
  $("#create-modal-name").val(cellData && cellData.writeName || "");
  $("#create-modal-yang-shang-ren").val(cellData && cellData.yangShangRen || "");
  $("#create-modal-wang-sheng-zhe").val(cellData && cellData.wangShengZhe || "");
  $("#create-modal-wish").val(cellData && cellData.wish || "");
  var $rowName = $("#create-modal-row-name");
  regionData.memoConfigTypes.indexOf(1) > -1 ? $rowName.show() : $rowName.hide();
  var $rowYangShangRen = $("#create-modal-row-yang-shang-ren");
  regionData.memoConfigTypes.indexOf(6) > -1 ? $rowYangShangRen.show() : $rowYangShangRen.hide();
  var $rowWangShengZhe = $("#create-modal-row-wang-sheng-zhe");
  regionData.memoConfigTypes.indexOf(7) > -1 ? $rowWangShengZhe.show() : $rowWangShengZhe.hide();
  var $rowWish = $("#create-modal-row-wish");
  regionData.memoConfigTypes.indexOf(2) > -1 ? $rowWish.show() : $rowWish.hide();
  ...
  if (cellData && cellData.contactList && cellData.contactList.length) {
    ...
    cellData.contactList.map(function (item, index) {
      ...
      if (!item.lunar) {
        $($contactBirthDate[index]).val(item.birth || "").show();
        $($contactLunarYear[index]).val(0).hide();
        $($contactLunarMonth[index]).val(0).hide();
        $($contactLunarDay[index]).val(0).hide();
        $($contactLunar[index]).prop({
          checked: !1
        });
      } else {
        $($contactBirthDate[index]).val("").hide();
        $($contactLunarYear[index]).val(item.year || 0).show();
        $($contactLunarMonth[index]).val(item.month || 0).show();
        $($contactLunarDay[index]).val(item.day || 0).show();
        $($contactLunar[index]).prop({
          checked: !0
        });
      }
    ...
    $("[data-create-contact-birth-date]").val("").show();
    $("[data-create-contact-lunar-year]").val(0).hide();
    $("[data-create-contact-lunar-month]").val(0).hide();
    $("[data-create-contact-lunar-day]").val(0).hide();
