...
$.seeView({
  ...
  onMouseEnterDetailCell: function (e, force) {
    ...
    var self = this, $this = $(e.target), disabled = !!parseInt($this.attr("data-disabled")), available = !!parseInt($this.attr("data-available")), online = !!parseInt($this.attr("data-online")), sequence = $this.attr("data-sequence"), row = parseInt($this.attr("data-row-index")), column = parseInt($this.attr("data-column-index"));
    if (disabled) return;
    if (available) {
      self.showHoverPopup(e, {}, sequence, row, column);
    } else if (data.cellData[data.currentRegionId] && data.cellData[data.currentRegionId][func.getRowColumnKey(row, column)]) {
      self.showHoverPopup(e, data.cellData[data.currentRegionId][func.getRowColumnKey(row, column)], sequence, row, column);
    } else {
  ...
  showHoverPopup: function (e, cellData, sequence, row, column) {
    var $this = $(e.target), pageX = e.pageX, pageY = e.pageY, $hover = $("#hover-popup");
    ...
    var hoverWidth = $hover.width(), hoverHeight = $hover.height(), hoverX = 0, hoverY = 0, showOnRight = !1;
  ...
  onClickDetailCell: function (e) {
    var self = this, $this = $(e.target), disabled = !!parseInt($this.attr("data-disabled")), online = !!parseInt($this.attr("data-online")), recorded = !!parseInt($this.attr("data-recorded")), row = parseInt($this.attr("data-row-index")), column = parseInt($this.attr("data-column-index")), sequence = $this.attr("data-sequence");
    if (disabled) return;
    ...
    if (!$this.hasClass("active")) {
      ...
      if ((recorded || online) && !data.cellData[data.currentRegionId][func.getRowColumnKey(row, column)]) {
