...
$.seeView({
  ...
  onClickActionFilter: function (e) {
    ...
    data.ordersFilter.remainDays = parseInt($("#filter-remain-days").val()) || "";
  ...
  onClickRowDetail: function (e) {
    ...
    $.seeAjax.post("detail", {
      ...
      if (res.success) {
        ...
        $("#detail-popup-title").text(item.buddhaName + " - " + item.wallName);
        ...
        $("#detail-popup-row-yang-shang-ren")[res.data.yangShangRen ? "show" : "hide"]();
        $("#detail-popup-wang-sheng-zhe").text(res.data.wangShengZhe || "");
        $("#detail-popup-row-wang-sheng-zhe")[res.data.wangShengZhe ? "show" : "hide"]();
  ...
  onClickRowDelete: function (e) {
    var self = this, $this = $(e.target), id = parseInt($this.attr("data-row-delete"));
