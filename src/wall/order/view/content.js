...
$.seeView({
  ...
  checkSelectAllStatus: function () {
    var allSelected = $("[data-row-select]").length == $("[data-row-select].active").length, $selectAll = $("#select-all");
    if (allSelected) $selectAll.addClass("active"); else $selectAll.removeClass("active");
  },
  onClickPageIndex: function (e) {
    var $this = $(e.target), currentPage = parseInt($this.attr("data-current-page")), pageIndex = parseInt($this.attr("data-page-index")), page = pageIndex == -1 ? currentPage - 1 : pageIndex == -2 ? currentPage + 1 : pageIndex;
  ...
  onClickActionHandle: function (e) {
    var self = this, $selected = $("[data-row-select].active");
    if (!$selected.length) {
  ...
  onClickActionExport: function (e) {
    var filter = [data.unhandledOrdersFilter, data.handledOrdersFilter][data.currentTabType - 1];
    window.open("/zzhadmin/buddhaWall_orderExcel/?beginDate=" + filter.startDate + "&endDate=" + filter.endDate + "&searchText=" + filter.search + "&wallId=" + filter.regionId + "&filterType=" + data.currentTabType);
  },
  onClickRowDetail: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-row-detail"));
    $.seeAjax.get("detail", {
      id: id
    }, function (res) {
      if (res.success) {
        ...
        if (feedImages) {
          feedImages.map(function (image) {
            $imageContainer.append(tpl.imageCell.render({
              image: image
            }));
          });
        }
        feedImages.length >= 10 && $("#detail-popup-feed-upload").parent().addClass("hide");
