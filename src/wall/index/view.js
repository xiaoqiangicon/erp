...
$.seeView({
  ...
  onClickActionAdd: function (e) {
    var $body = $("body"), $modal = $("#select-modal"), $modalContent = $("#select-modal-content");
    if ($("[data-template-cell]").length) {
      ...
      $.seeAjax.get("templates", {}, function (res) {
        if (res.success) {
          $modalContent.append(tpl.templateCell.render(data.customTemplateData));
  ...
  onClickSelectModalOk: function (e) {
    ...
    location.href = "/zzhadmin/createBuddhaIndex/" + (id ? "?templateId=" + id : "");
  },
  onMouseEnterUnitRegion: function (e) {
    var $this = $(e.target), pageX = e.pageX, pageY = e.pageY, $hoverBody = $("#hover-popup-body"), regions = $this.attr("data-regions").split(",");
    $hoverBody.html("");
    regions.map(function (item) {
      $hoverBody.append(tpl.hoverRow.render({
        name: item
      }));
    });
    var $hover = $("#hover-popup"), hoverWidth = $hover.width(), hoverHeight = $hover.height(), hoverX = 0, hoverY = 0, showOnRight = !1;
  ...
  onClickUnitCopy: function (e) {
    var $this = $(e.target), id = parseInt($this.attr("data-unit-copy"));
