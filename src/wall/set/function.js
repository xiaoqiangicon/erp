...
func.init = function () {
  $.seeAjax.get("tags", {}, function (res) {
    if (res.success) {
      var $tagPopupMy = $("#tag-popup-my"), $tagPopupSystem = $("#tag-popup-system");
      res.data.custom && res.data.custom.length && res.data.custom.map(function (item) {
        data.tags[item.id] = item;
        item.type = 2;
        $tagPopupMy.append(tpl.tagPopupCell.render(item));
      });
      res.data.system && res.data.system.length && res.data.system.map(function (item) {
        data.tags[item.id] = item;
        item.type = 1;
  ...
  $.seeAjax.get("buddhas", {}, function (res) {
    if (res.success) {
      ...
      res.data && res.data.length && res.data.map(function (item) {
        if (item.category == 2) $inputPaiWei.append(tpl.buddhaCell.render(item)); else $inputBuddha.append(tpl.buddhaCell.render(item));
...
func.renderPage = function (res) {
  ...
  var $inputBuddha = $("#input-buddha");
  var $inputPaiWei = $("#input-pai-wei");
  var $contentRowBuddha = $("#content-row-buddha"), $contentRowPaiWei = $("#content-row-pai-wei");
  ...
  var tagsHtml = "";
  res.data.tags && res.data.tags.length && res.data.tags.map(function (tagId) {
  ...
  if (res.data.cover) {
    $coverImageAdd.before(tpl.coverImageCell.render({
  ...
  if (res.data.previewImages) {
    ...
    previewImagesArray.map(function (image) {
      $previewImageAdd.before(tpl.previewImageCell.render({
        image: image
      }));
    });
  ...
  res.data.memoConfig && res.data.memoConfig.length && res.data.memoConfig.map(function (item) {
  ...
  if (typeof res.data.usePrinter != "undefined") {
    $("input[name=\"printer-popup-activate\"][value=\"" + res.data.usePrinter + "\"]").prop({
      checked: !0
    });
    res.data.printers && res.data.printers.split(",").map(function (printerId) {
      $("[data-printer-cell-input=\"" + printerId + "\"]").prop({
        checked: !0
      });
    });
    $("#printer-popup-pages").val(res.data.printPages);
    $("input[name=\"printer-popup-print-type\"][value=\"" + res.data.printType + "\"]").prop({
      checked: !0
    });
  ...
  if (res.data.shareImage) {
    $shareImageAdd.before(tpl.shareImageCell.render({
