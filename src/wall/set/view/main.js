...
$.seeView({
  ...
  afterUploadCover: function (res) {
    ...
    $uploadEl.before(tpl.coverImageCell.render({
      image: res[0].src
    }));
    $uploadEl.hide();
  },
  ...
  afterUploadPreviewImage: function (res) {
    ...
    res.map(function (item) {
      $uploadEl.before(tpl.previewImageCell.render({
        image: item.src
      }));
    });
  ...
  onChangeContentPay: function (e) {
    var usePay = !!parseInt($("input[name=\"content-pay\"]:checked").attr("value"));
  ...
  onClickMemoConfig: function (e) {
    var $this = $(e.target), length = parseInt($this.attr("data-length")) || "", disabled = !!parseInt($this.attr("data-disabled"));
    if (disabled) return;
    var type = parseInt($this.attr("data-memo-config")), name = $("[data-memo-name=\"" + type + "\"]").text();
  ...
  afterUploadShare: function (res) {
    ...
    $uploadEl.before(tpl.shareImageCell.render({
      image: res[0].src
    }));
    $uploadEl.hide();
  },
  ...
  onChangeInputCategory: function (e) {
    var $this = $(e.target), value = parseInt($this.val()), $contentRowBuddha = $("#content-row-buddha"), $contentRowPaiWei = $("#content-row-pai-wei");
    if (value == 2) {
