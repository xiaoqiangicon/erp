...
function onInputChange() {
  var value = $tagPopupInput.val(), tagsArray = value.split(" "), lastTag = tagsArray[tagsArray.length - 1];
  if (value.length > data.lastTagPopupInputLength) {
    if (lastTag.length == 4) {
      $tagPopupInput.val(value + " ");
    } else if (lastTag.length > 4) {
      var lastTagArray = [], i, il;
      ...
      $tagPopupInput.val(value + (tagsArray[tagsArray.length - 1].length < 4 ? "" : " "));
    }
  }
  data.lastTagPopupInputLength = value.length;
  $("[data-tag-popup-cell]").map(function () {
    var $this = $(this);
    tagsArray.indexOf($this.attr("data-name")) > -1 ? $this.removeClass("unselected") : $this.addClass("unselected");
  });
  setTimeout(function () {
    moveCursorToEnd();
  }, 0);
...
function moveCursorToEnd() {
  ...
  if (document.selection) {
    ...
    sel.collapse();
    sel.select();
  } else if (typeof tagPopupInput.selectionStart == "number" && typeof tagPopupInput.selectionEnd == "number") {
