...
$.seeView({
  ...
  onClickContentEditTag: function (e) {
    ...
    $("[data-tag-popup-cell]").map(function () {
      var $this = $(this), id = parseInt($this.attr("data-tag-popup-cell"));
      if (currentTagIds.indexOf(id) > -1) $this.removeClass("unselected"); else $this.addClass("unselected");
    });
  ...
  onClickTagPopupCell: function (e) {
    var $this = $(e.currentTarget), tagName = $this.attr("data-name"), $input = $("#tag-popup-input");
    var newValue;
  ...
  onClickTagPopupCellDelete: function (e) {
    e.stopPropagation();
    var $this = $(e.target), id = parseInt($this.attr("data-tag-popup-cell-delete")), name = $this.parent().attr("data-name"), $input = $("#tag-popup-input"), input = $input.val(), tagsArray = input.split(" "), index = tagsArray.indexOf(name);
    commonFunc.confirm("确定删除这个标签吗", function () {
      $.seeAjax.post("deleteTag", {
        id: id
      }, function (res) {
        if (res.success) {
          $this.parent().remove();
  ...
  onClickTagPopupOk: function (e) {
    var self = this, $this = $(e.target), handling = $this.attr("data-handling") && !!parseInt($this.attr("data-handling")), $input = $("#tag-popup-input"), tagsArray = _.without(_.uniq($input.val().split(" ")), "");
    if (handling) return;
    ...
    if (tagBeyond4length) {
      commonFunc.dialog("标签 " + tagBeyond4length + " 超过4个字，请修改后再保存");
      return;
    }
    var allTagsArray = [];
    var allTagIdsArray = [];
