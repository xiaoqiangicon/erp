...
$.seeView({
  ...
  onClickActionSave: function (e) {
    var self = this, $this = $("#action-save"), handling = $this.attr("data-handling") && !!parseInt($this.attr("data-handling"));
    if (handling) return;
  ...
  saveOtherSiteAllImagesSuccess: function () {
    if (data.editAction) {
      ...
      $.seeAjax.post("edit", window.contentData, function (res) {
        $("#action-save").attr({
          "data-handling": 0
        }).text("保存");
        zzhHandling.hide();
      ...
      $.seeAjax.post("create", window.contentData, function (res) {
        $("#action-save").attr({
          "data-handling": 0
        }).text("保存");
        if (res.success) {
  ...
  onKeepData: function () {
    ...
    var buddhaId = category == 2 ? parseInt($("#input-pai-wei").val()) : parseInt($("#input-buddha").val());
    if (!buddhaId) {
    ...
    if (!$("[data-memo-checkbox=\"1\"]").prop("checked") && !$("[data-memo-checkbox=\"6\"]").prop("checked") && !$("[data-memo-checkbox=\"7\"]").prop("checked")) {
      commonFunc.alert("功德芳名、阳上人、往生者请至少选择一个");
      return !1;
    }
    var memoConfig = [];
    $("[data-memo-checkbox]:checked").map(function () {
      var $this = $(this), type = parseInt($this.attr("data-memo-checkbox"));
      memoConfig.push({
        type: type,
        length: parseInt($("[data-memo-config=\"" + type + "\"]").attr("data-length")) || 0,
        tip: $("[data-memo-tip=\"" + type + "\"]").val(),
        alias: $("[data-memo-alias=\"" + type + "\"]").val() || ""
      });
    });
    var usePrinter = parseInt($("[name=\"printer-popup-activate\"]:checked").attr("value"));
    var printers = [];
    $("[data-printer-cell-input]:checked").map(function () {
      printers.push(parseInt($(this).attr("data-printer-cell-input")));
    });
    var printType = parseInt($("[name=\"printer-popup-print-type\"]:checked").attr("value"));
    ...
    var shareImage = $shareImage.length && $($shareImage[0]).attr("data-image") || "";
    if (!shareImage) {
