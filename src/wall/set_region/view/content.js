...
$.seeView({
  ...
  onClickDetailCell: function (e) {
    var $this = $(e.target), row = parseInt($this.attr("data-row-index")), column = parseInt($this.attr("data-column-index"));
    var $cells;
    if (row > 0 && column > 0) {
      $this.toggleClass("active");
    } else if (row > 0 && column == 0) {
      $cells = $("[data-detail-cell][data-row-index=\"" + row + "\"]");
      if ($this.hasClass("active")) $cells.removeClass("active"); else $cells.addClass("active");
    } else if (row == 0 && column > 0) {
  ...
  checkToShowPriceEdit: function () {
    if ($("[data-detail-cell=\"1\"].active").length) $contentPrices.show(); else $contentPrices.hide();
  },
  onClickPriceRowDelete: function (e) {
    $(e.target).parents("[data-price-row]").remove();
  },
  onClickPriceRowSet: function (e) {
    var $this = $(e.target), index = parseInt($this.attr("data-price-row-set")), value = $this.attr("data-value");
  ...
  onMouseEnterDetailCell: function (e) {
    var self = this, $this = $(e.target), sequence = $this.attr("data-sequence"), disabled = !!parseInt($this.attr("data-disabled")), id = parseInt($this.attr("data-type")), row = parseInt($this.attr("data-row-index")), column = parseInt($this.attr("data-column-index"));
