...
$.seeView({
  ...
  getEditPrices: function () {
    var $rows = $("[data-price-row]"), $types = $("[data-price-row-type]"), $prices = $("[data-price-row-price]"), $basePrices = $("[data-price-row-set]");
  ...
  checkUnUsedPrices: function () {
    Object.keys(data.priceData).map(function (key) {
      var id = parseInt(key);
      !$("[data-detail-cell=\"1\"][data-type=\"" + id + "\"]").length && delete data.priceData[key];
    });
  },
  afterBindPrices: function () {
    $("[data-price-row]").map(function (index) {
      var $row = $(this);
      if (index > 0) $row.remove(); else {
  ...
  onClickActionSave: function (e) {
    var $this = $("#action-save"), handling = $this.attr("data-handling") && !!parseInt($this.attr("data-handling"));
    if (handling) return;
    var priceList = [];
    Object.keys(data.priceData).map(function (key) {
      var id = parseInt(key);
      ...
      $("[data-detail-cell=\"1\"][data-type=\"" + id + "\"]").map(function () {
        var $this = $(this), row = parseInt($this.attr("data-row-index")), column = parseInt($this.attr("data-column-index"));
  ...
  onClickSeatsPopupOk: function (e) {
    var self = this, $this = $("#seats-popup-ok"), handling = $this.attr("data-handling") && !!parseInt($this.attr("data-handling"));
    if (handling) return;
  ...
  renderRegion: function (seats) {
    seats.map(function (rowItem, rowIndex) {
      ...
      rowItem.split("|").map(function (columnItem, columnIndex) {
        ...
        var $item = $("[data-detail-cell][data-row-index=\"" + row + "\"][data-column-index=\"" + column + "\"]");
        if (columnItem == "_") $item.removeClass("available").addClass("disabled"); else $item.removeClass("disabled").addClass("available");
      });
    });
  }
});
